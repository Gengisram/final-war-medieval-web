...
FWM.nube = (function () {
  ...
  async function registrar(correo, clave, nombre) {
    ...
    if (FWM.cobro && usuario) FWM.cobro.cambiarUsuario(usuario.id);
    return usuario;
  }
  ...
  async function salir() { if (cliente) await cliente.auth.signOut(); usuario = null; perfil = null; if (FWM.cobro) FWM.cobro.cambiarUsuario(null); }
  ...
  // 13 sep 2026: `nivel` es el de tu MEJOR facción (la Leyenda pide nivel 10 con cualquiera) y el avatar, la clase de tu facción.
  async function guardarHeroe(h) {
    if (!cliente || !usuario) return false;
    const nivel = FWM.heroe ? FWM.heroe.mejorNivel() : 1;
    const avatar = FWM.heroe ? FWM.heroe.clase() : "espadachin";
    const { error } = await cliente.from("perfiles").update({ heroe: h, nivel, avatar }).eq("id", usuario.id);
    if (error) throw error;
    perfil = Object.assign({}, perfil, { heroe: h, nivel, avatar });
    return true;
  }
  ...
  // La columna `faccion` llega con la parte 10 del SQL. Si aún no está, la partida se guarda igual, sin ella.
  const sinColumna = (error) => !!(error && (error.code === "PGRST204" || error.code === "42703" || /column/i.test(error.message || "")));
  async function insertarPartidas(filas) {
    let { error } = await cliente.from("partidas").insert(filas);
    if (sinColumna(error)) ({ error } = await cliente.from("partidas").insert(filas.map(f => { const c = Object.assign({}, f); delete c.faccion; return c; })));
    return error;
  }
  async function enviarPartida(p) {
    const fila = { puntos: p.puntos | 0, tipo: p.tipo || "rapida", turnos: p.turnos | 0, gano: !!p.gano, semilla: p.semilla | 0, mapa: p.mapa || null, faccion: p.faccion || null };
    if (!cliente || !usuario) { guardarPendiente(Object.assign({ fecha: new Date().toISOString() }, fila)); return false; }
    const error = await insertarPartidas([Object.assign({ usuario: usuario.id }, fila)]);
  ...
  async function enviarPendientes() {
    ...
    const filas = l.map(f => ({ usuario: usuario.id, puntos: f.puntos, tipo: f.tipo, turnos: f.turnos, gano: f.gano, semilla: f.semilla, mapa: f.mapa, faccion: f.faccion || null, fecha: f.fecha || undefined }));
    const error = await insertarPartidas(filas);
    if (error) return 0;
  ...
  // "Informar de un problema o sugerencia" (25 sep 2026). Si no hay conexión, se guarda y se manda al volver.
  const CLAVE_REPORTES = "fwm.reportesPendientes";
  function reportesPendientes() { try { return JSON.parse(localStorage.getItem(CLAVE_REPORTES) || "[]"); } catch (e) { return []; } }
  async function reportar(fila) {
    const completa = Object.assign({ aparato: aparato(), usuario: usuario ? usuario.id : null }, fila);
    try {
      if (!cliente) throw new Error("sin nube");
      const { error } = await cliente.from("reportes").insert(completa); if (error) throw error;
      return "enviado";
    } catch (e) {
      const l = reportesPendientes(); l.push(completa); while (l.length > 5) l.shift(); // con la partida dentro pesan: pocos
      try { localStorage.setItem(CLAVE_REPORTES, JSON.stringify(l)); } catch (e2) { l.forEach(x => { x.partida = null; }); try { localStorage.setItem(CLAVE_REPORTES, JSON.stringify(l)); } catch (e3) { /* nada */ } }
      return "guardado";
    }
  }
  async function enviarReportesPendientes() {
    if (!cliente) return; const l = reportesPendientes(); if (!l.length) return;
    const quedan = [];
    for (const f of l) { try { const { error } = await cliente.from("reportes").insert(f); if (error) quedan.push(f); } catch (e) { quedan.push(f); } }
    try { if (quedan.length) localStorage.setItem(CLAVE_REPORTES, JSON.stringify(quedan)); else localStorage.removeItem(CLAVE_REPORTES); } catch (e) { /* nada */ }
  }
  ...
  // Día de vida de este aparato: 0 el día que instaló, 1 al día siguiente… Es lo que permite calcular
  // la retención sin guardar nada personal: basta con saber en qué día de vida ocurrió cada evento.
  function diaDeVida() {
    try {
      let d = localStorage.getItem("fwm.instalado");
      if (!d) { d = new Date().toISOString().slice(0, 10); localStorage.setItem("fwm.instalado", d); }
      return Math.max(0, Math.round((Date.parse(new Date().toISOString().slice(0, 10)) - Date.parse(d)) / 864e5));
    } catch (e) { return null; }
  }
  function evento(tipo, datos) {
    if (!cliente) return;
    // a todos los eventos se les añade el día de vida, la versión y el idioma: sin eso no se puede
    // medir la retención ni saber si una versión nueva mejora o empeora (11 sep 2026)
    const comunes = { dia: diaDeVida(), v: FWM.VERSION || null, idioma: (FWM.idioma && FWM.idioma.actual && FWM.idioma.actual()) || null };
    try { cliente.from("eventos").insert({ aparato: aparato(), usuario: usuario ? usuario.id : null, tipo, datos: Object.assign(comunes, datos || {}) }).then(() => {}, () => {}); } catch (e) { /* nada */ }
  }
  ...
  // La Leyenda de Final War (parte 10 del SQL): { usuario, nombre, puntos, faccion, desde } o null.
  // La función del servidor además apunta los cambios en la Sala de Leyendas. Sin ella, se calcula aquí con
  // el ranking total y el nivel de los perfiles (sin los 12 meses ni la sala).
  async function leyendaActual() {
    if (!cliente) return null;
    const { data, error } = await cliente.rpc("leyenda_actual");
    if (!error) { const f = Array.isArray(data) ? data[0] : data; return f && f.usuario ? f : null; }
    const top = await ranking("total", 20, "suma");
    if (!top.length) return null;
    const perfiles = await perfilesDe(top.map(f => f.usuario));
    const f = top.find(x => perfiles[x.usuario] && Number(perfiles[x.usuario].nivel) >= 10);
    return f ? { usuario: f.usuario, nombre: f.nombre, puntos: Number(f.puntos), faccion: perfiles[f.usuario].heroe && perfiles[f.usuario].heroe.faccion } : null;
  }
  async function salaLeyendas(limite) {
    if (!cliente) return [];
    const { data, error } = await cliente.from("leyendas").select("usuario, nombre, faccion, desde, hasta").order("desde", { ascending: false }).limit(limite || 50);
    if (error) return [];
    return data || [];
  }
