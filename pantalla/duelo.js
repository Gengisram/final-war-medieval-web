...
// El mismo módulo lleva el modo Resistir a dobles (9 sep 2026): dos jugadores humanos contra las hordas,
...
FWM.duelo = (function () {
  ...
  const esSala = () => !!(sesion && sesion.partida); // batalla montada por el anfitrión, con máquinas o sin ellas
  ...
  function retar(app, codigo, alEstado, alEmpezar, opciones) {
    ...
    // Un solo canal por código, venga de donde venga (14 sep 2026): el código de Resistir a dobles se metía en
    ...
    // sala (22 sep 2026): el anfitrión ha montado una batalla en el formulario y la manda entera por el canal.
    // Quien escribe el código juega LO QUE HAYA MONTADO EL OTRO, venga de la pantalla que venga.
    const partida = (opciones && opciones.partida) || null;
    const coop = !!(opciones && opciones.coop) || !!partida; const prefijo = "reto:";
    ...
    cola.on("presence", { event: "sync" }, () => {
      ...
      const ses = { id: (partida ? "sala-" : coop ? "coop-" : "reto-") + codigo + "-" + Date.now().toString(36), semilla: Math.floor(Math.random() * 1e6) + 1, rol: "anfitrion", rival: { uid: inv.uid, nombre: inv.nombre, avatar: inv.avatar, faccion: inv.faccion, heroe: inv.heroe, elo: inv.elo }, miElo: m.elo, igualar, coop, partida };
      canalReto.send({ type: "broadcast", event: "empezar", payload: { a: inv.uid, de: m.uid, id: ses.id, semilla: ses.semilla, nombre: m.nombre, avatar: m.avatar, faccion: m.faccion, heroe: m.heroe, elo: m.elo, igualar, coop, partida, proto: FWM.PROTOCOLO } });
      setTimeout(() => arrancar(ses), 300);
    });
    cola.on("broadcast", { event: "empezar" }, ({ payload }) => {
      if (emparejado || payload.a !== m.uid) return;
      if ((payload.proto || 0) !== FWM.PROTOCOLO) { alEstado(App.datos.textos.duelo.otraVersion, null, codigo, creador); return; }
      arrancar({ id: payload.id, semilla: payload.semilla, rol: "invitado", rival: { uid: payload.de, nombre: payload.nombre, avatar: payload.avatar, faccion: payload.faccion, heroe: payload.heroe, elo: payload.elo }, miElo: m.elo, igualar: !!payload.igualar, coop: !!payload.coop, partida: payload.partida || null });
    });
  ...
  let yoListo = false, rivalListo = false, alListos = null, firmaInicial = null;
  // la firma del tablero recién montado, que es lo que se compara con el otro teléfono
  function mandarInicio() {
    if (!canal || !App || !App.estado || App.estado.turno > 1) return;
    firmaInicial = firma(App.estado);
    canal.send({ type: "broadcast", event: "inicio", payload: { firma: firmaInicial } });
  }
  function empezar(app, ses, conCaraACara) {
    ...
    App.accionesTurno = []; firmaInicial = null;
    if (enLinea()) { conectar(); setTimeout(mandarInicio, 800); } // por si el canal tardó en engancharse
    if (conCaraACara) {
  ...
  function conectar() {
    ...
    // La misma partida en los dos teléfonos: al empezar, cada uno manda la firma de su tablero. Si no cuadran,
    // se dice y se deja la partida en vez de romperse a mitad con las tropas descuadradas (22 sep 2026).
    canal.on("broadcast", { event: "inicio" }, ({ payload }) => {
      if (!App.estado || App.estado.turno > 1 || firmaInicial == null) return;
      if (payload && payload.firma !== firmaInicial) anular(App.datos.textos.duelo.noCuadra);
    });
    ...
    canal.subscribe(async (estado) => { if (estado === "SUBSCRIBED") { await canal.track({ uid: uid(), desde: Date.now() }); if (yoListo) canal.send({ type: "broadcast", event: "listo", payload: {} }); mandarInicio(); pedirTurno(); } });
    arrancarLatido();
  }
  ...
  function rendirse(motivo) {
    ...
    const rival = esCoop() ? App.estado.jugadores.find(j => j.horda) : App.estado.jugadores.find(j => j.id !== App.humano);
  ...
  function caraACara(app, alAcabar) {
    ...
    // en una sala pueden jugar máquinas además de los dos amigos: en el cara a cara salen ellos dos y las
    // máquinas se cuentan en la línea de reglas (si no, salían cinco retratos en fila)
    const sala = esSala();
    const maquinas = sala ? e.jugadores.filter(j => !j.humano && !j.remoto && !j.horda).length : 0;
    e.jugadores.filter(j => !j.horda && (!sala || j.humano || j.remoto)).forEach((j, i) => {
    ...
    const horda = coop && e.jugadores.find(j => j.horda);
    if (horda) {
      ...
      const niv = document.createElement("div"); niv.className = "cara-nivel"; niv.textContent = T.resistir.boton; lado.appendChild(niv);
      const n = document.createElement("div"); n.className = "cara-nombre"; n.textContent = horda.nombre; lado.appendChild(n);
      fila.appendChild(lado);
    }
    caja.appendChild(fila);
    const reglas = document.createElement("div"); reglas.className = "cara-reglas";
    reglas.textContent = (coop ? T.coop.reglasLinea : T.duelo.reglasLinea).replace("{turnos}", e.limiteTurnos).replace("{s}", SEG_TURNO)
      + (maquinas ? " · " + T.batallas.maquinas.replace("{n}", maquinas) : "");
    ...
    const rivalId = (App.estado.jugadores.find(j => j.remoto) || App.estado.jugadores.find(j => j.id !== App.humano)).id;
  ...
  const nombreRival = () => (sesion && sesion.rival && sesion.rival.nombre) || "";
  const esAmigo = () => !!(sesion && /^(reto|coop|sala)-/.test(String(sesion.id))); // dobles también es por código, entre amigos
  ...
  function alAcabar(estadoFinal) {
    ...
    const gane = FWM.victoria.haGanado(estadoFinal, App.humano);
