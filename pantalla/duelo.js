...
FWM.duelo = (function () {
  ...
  function retar(app, codigo, alEstado, alEmpezar, opciones) {
    ...
    const coop = !!(opciones && opciones.coop); const prefijo = coop ? "coop:" : "reto:";
    ...
    cola.on("presence", { event: "sync" }, () => {
      ...
      const ses = { id: (coop ? "coop-" : "reto-") + codigo + "-" + Date.now().toString(36), semilla: Math.floor(Math.random() * 1e6) + 1, rol: "anfitrion", rival: { uid: inv.uid, nombre: inv.nombre, avatar: inv.avatar, heroe: inv.heroe, elo: inv.elo }, miElo: m.elo, igualar, coop };
      canalReto.send({ type: "broadcast", event: "empezar", payload: { a: inv.uid, de: m.uid, id: ses.id, semilla: ses.semilla, nombre: m.nombre, avatar: m.avatar, heroe: m.heroe, elo: m.elo, igualar, coop, proto: FWM.PROTOCOLO } });
      setTimeout(() => arrancar(ses), 300);
    });
    cola.on("broadcast", { event: "empezar" }, ({ payload }) => {
      if (emparejado || payload.a !== m.uid) return;
      if ((payload.proto || 0) !== FWM.PROTOCOLO) { alEstado(App.datos.textos.duelo.otraVersion, null, codigo); return; }
      arrancar({ id: payload.id, semilla: payload.semilla, rol: "invitado", rival: { uid: payload.de, nombre: payload.nombre, avatar: payload.avatar, heroe: payload.heroe, elo: payload.elo }, miElo: m.elo, igualar: !!payload.igualar, coop: !!payload.coop });
    });
