...
const App = (function () {
  ...
  App.iniciar = function () {
    ...
    document.getElementById("btn-objetivo").addEventListener("click", () => App.abrirObjetivo());
    ...
    // el mismo botón en el inicio (19 sep 2026): allí no hay mapa ni escudo antitoque, con "click" basta
    const bi = document.getElementById("btn-musica-inicio"); if (bi) bi.addEventListener("click", (ev) => { ev.stopPropagation(); App.alternarMusica(); });
    FWM.paneles.pintarBotonMusica();
  ...
  App.nuevaPartida = function (op) {
    { const c = document.getElementById("ceremonia"); if (c) c.remove(); } // la pantalla final de la partida anterior, si quedó
    App.avisosGuion = []; // y los avisos que quedaron sin enseñar: salían al empezar la siguiente
    FWM.sonido.inicio();
    FWM.inicio.ocultar();
    ...
    // Facciones (13 sep 2026): el humano juega con su facción (la elegida en el inicio, o la que pida el modo:
    // la de la campaña, la del reto). Las IAs cogen facciones de las seis sin repetir mientras haya: son el mejor
    // escaparate de las de pago. En las Grandes batallas las IAs son los reinos del mapa, cada uno en su tierra, y
    // tú juegas con tu facción en la tierra que elijas. Los modos pueden fijar los enemigos (op.enemigos).
    ...
    const listaF = FWM.facciones.lista();
    let miFaccion = op.faccion && App.datos.facciones[op.faccion] ? op.faccion : FWM.heroe.faccion();
    if (!FWM.heroe.tiene(miFaccion) && !(op.duelo || op.coop)) miFaccion = FWM.heroe.tiene(FWM.heroe.faccion()) ? FWM.heroe.faccion() : "castilla";
    const miHeroe = FWM.heroe.paraPartida(op, miFaccion);
    const gastados = op.gastadosIA != null ? op.gastadosIA : FWM.heroes.puntosGastados(miHeroe);
    const tierraHumano = deEscenario ? (deEscenario.includes(op.bando) ? op.bando : gb.elegir(deEscenario)) : null;
    const reinosLibres = deEscenario ? gb.barajar(deEscenario.filter(b => b !== tierraHumano)) : [];
    const faccionesLibres = gb.barajar(listaF.filter(f => f !== miFaccion));
    // el reino se llama como su facción o su bando; si se repite, lleva ordinal
    const usados = {};
    const nombreDe = (nombre) => { usados[nombre] = (usados[nombre] || 0) + 1; return nombre + (usados[nombre] > 1 ? " " + ["", "", "II", "III", "IV", "V", "VI", "VII"][usados[nombre]] : ""); };
    const COLORES = paleta();
    const jugadores = [{ nombre: nombreDe(App.datos.facciones[miFaccion].nombre), color: COLORES[0], humano: true, nombreCapital: op.capital || null, faccion: miFaccion, nivel: miHeroe.nivel, heroe: miHeroe, tierra: tierraHumano }];
    const apodos = gb.barajar((App.datos.nombresIA || []).slice());
    // el jefe rival se llama como su gente (un noruego no se llama Teresa); sin lista, un nombre de la general
    const apodosUsados = new Set();
    const apodoDe = (cultura, i) => {
      const pool = ((App.datos.nombresPorCultura || {})[cultura] || []).filter(x => !apodosUsados.has(x));
      const r = gb.siguiente();
      const n = pool.length ? pool[Math.floor(r * pool.length)] : (apodos.find(x => !apodosUsados.has(x)) || ("IA " + i));
      apodosUsados.add(n); return n;
    };
    for (let i = 1; i < n; i++) {
      // en Resistir la horda es SIEMPRE el último jugador; en dobles, el 1 es el compañero humano
      if (op.tipo === "resistir" && i === n - 1) { jugadores.push({ nombre: App.datos.bandos.hordas.nombre, color: "#6b6455", humano: false, bando: "hordas", apodo: App.datos.bandos.hordas.nombre }); continue; }
      // sala: el puesto 1 es el amigo y lo rellena la sesión más abajo. No se le sortea facción ni nombre aquí,
      // que si no gastaba uno de la lista y el siguiente reino salía con "II" detrás (22 sep 2026).
      if (op.sala && i === 1) { jugadores.push({ nombre: "", color: COLORES[1], humano: false }); continue; }
      const enemigo = (op.enemigos || [])[i - 1] || null;
      // el jefe de campaña lleva un héroe de leyenda con todos los puntos gastados, no uno de mi nivel
      const esJefe = op.jefe && i === 1;
      // en una sala con un amigo, las máquinas se montan con los números del anfitrión (op.nivelIA / op.gastadosIA):
      // si cada teléfono usara SU héroe saldrían máquinas distintas en cada pantalla y la partida se rompería
      const nivelIA = esJefe ? 10 : (op.nivelIA || miHeroe.nivel);
      // reino de los de siempre, con su nombre propio (enemigos de las campañas), o reino del mapa en las grandes batallas
      if (enemigo && enemigo.bando && !enemigo.faccion && App.datos.bandos[enemigo.bando]) {
        const jf = esJefe ? op.jefe : null;
        const hIA = esJefe ? FWM.heroe.paraIA(op.semilla + 777, 10, 40, jf.clase) : FWM.heroe.paraIA(op.semilla + 1000 * i, nivelIA, gastados);
        jugadores.push({ nombre: nombreDe(enemigo.reino || (jf && jf.reino) || App.datos.bandos[enemigo.bando].nombre), color: enemigo.color || COLORES[i % COLORES.length], humano: false, bando: enemigo.bando, nombresDe: enemigo.nombresDe || null, nombreCapital: enemigo.capital || null, nivel: nivelIA, apodo: enemigo.apodo || apodoDe(enemigo.cultura || enemigo.bando, i), heroe: hIA });
        continue;
      }
      if (deEscenario && !(enemigo && enemigo.faccion)) {
        const b = reinosLibres.length ? reinosLibres.shift() : gb.elegir(deEscenario);
        const hIA = FWM.heroe.paraIA(op.semilla + 1000 * i, nivelIA, gastados);
        jugadores.push({ nombre: nombreDe(App.datos.bandos[b].nombre), color: COLORES[i % COLORES.length], humano: false, bando: b, nivel: nivelIA, apodo: apodoDe(b, i), heroe: hIA, tierra: b });
        continue;
      }
      const f = (enemigo && enemigo.faccion && App.datos.facciones[enemigo.faccion]) ? enemigo.faccion : (faccionesLibres.length ? faccionesLibres.shift() : gb.elegir(listaF));
      const hIA = esJefe ? FWM.heroe.paraIA(op.semilla + 777, 10, 40, App.datos.facciones[f].clase) : FWM.heroe.paraIA(op.semilla + 1000 * i, nivelIA, gastados, App.datos.facciones[f].clase);
      jugadores.push({ nombre: nombreDe((enemigo && enemigo.reino) || App.datos.facciones[f].nombre), color: (enemigo && enemigo.color) || COLORES[i % COLORES.length], humano: false, faccion: f, nombresDe: (enemigo && enemigo.nombresDe) || null, nombreCapital: (enemigo && enemigo.capital) || null, nivel: nivelIA, apodo: (enemigo && enemigo.apodo) || apodoDe((enemigo && enemigo.cultura) || f, i), heroe: hIA });
    }
    // duelo: el anfitrión es el jugador 0 y el invitado el 1 en los dos teléfonos (misma semilla, mismas facciones);
    ...
    let miIndice = 0;
    // Resistir a dobles y duelos: cada uno lleva SU facción (viene en la sesión). El anfitrión es el 0.
    const red = op.coop || op.duelo;
    if (red) {
      const d = red; miIndice = d.rol === "invitado" ? 1 : 0; const otro = 1 - miIndice;
      const mio = FWM.nube.nombre() || FWM.guardado.ajustes().nombre || App.datos.textos.fichas.tuMayus;
      // la facción del otro viaja en la presencia y en el "empezar"; si faltara, la del héroe que manda (14 sep 2026: la
      // presencia no la llevaba y quien creaba el código veía a su compañero como Castilla: partidas distintas)
      const fMandada = d.rival.faccion || (d.rival.heroe && d.rival.heroe.faccion);
      const fRival = fMandada && App.datos.facciones[fMandada] ? fMandada : (d.rol === "bot" ? gb.elegir(listaF) : "castilla");
      // cada uno con su héroe tal cual (viene en la sesión); el bot, uno de mi nivel. En el reto entre amigos,
      // si quien crea el código marca "Igualar héroes", los dos juegan al nivel y puntos del más bajo, sin objetos.
      let hMio = miHeroe;
      let hRival = d.rival.heroe ? Object.assign({}, d.rival.heroe, { faccion: fRival, clase: App.datos.facciones[fRival].clase }) : FWM.heroe.paraIA(op.semilla + 4242, miHeroe.nivel, gastados, App.datos.facciones[fRival].clase);
      if (op.duelo && d.igualar) [hMio, hRival] = FWM.heroes.igualar(hMio, hRival);
      // a dobles el compañero no va de rojo, que es el color del enemigo: el invitado va de verde (14 sep 2026)
      const color = (i) => op.coop && i === 1 ? COLORES[2] : COLORES[i];
      const yo = { faccion: miFaccion, nivel: hMio.nivel, humano: true, remoto: false, apodo: mio, heroe: hMio, color: color(miIndice), nombre: App.datos.facciones[miFaccion].nombre };
      const el = { faccion: fRival, nivel: hRival.nivel || 1, humano: false, remoto: d.rol !== "bot", apodo: d.rival.nombre, heroe: hRival, color: color(otro), nombre: App.datos.facciones[fRival].nombre + (fRival === miFaccion ? " II" : "") };
      jugadores[miIndice] = Object.assign({}, jugadores[miIndice], yo);
      jugadores[otro] = Object.assign({}, jugadores[otro], el);
      if (miIndice === 1 && fRival === miFaccion) { jugadores[0].nombre = App.datos.facciones[fRival].nombre; jugadores[1].nombre = App.datos.facciones[miFaccion].nombre + " II"; }
    }
    // escenario: el mapa manda dónde empieza cada reino (tu tierra, la que has elegido)
    if (deEscenario) mapa = Object.assign({}, mapa, { inicios: jugadores.map(j => mapa.porBando[j.tierra]).filter(Boolean) });
    // equipos: por defecto cada uno va por su cuenta; en cooperativo, los dos humanos juntos (21 sep 2026)
    jugadores.forEach((j, i) => { if (j.equipo == null) j.equipo = (op.equipos && op.equipos[i] != null) ? op.equipos[i] : (op.coop && i <= 1 ? 0 : i); });
    // tipo de batalla: con qué empieza cada uno, objetivo y guion (modo/batallas.js)
    const conf = FWM.batallas.preparar(op.batalla || (op.tipo === "resistir" ? "resistir" : "conquista"), jugadores, op);
    const guion = (op.guion || []).concat(conf.guion || []);
    App.estado = FWM.motor.crearPartida({ datos: App.datos, mapa, jugadores, semilla: op.semilla, todasTecnologias: op.tecnologia === "todo", multiplicadorHucha: op.hucha || 1, limiteTurnos: op.limite || conf.limiteTurnos || 0, sinCarreteras: true, objetivo: op.objetivo || conf.objetivo, guion });
    App.estado.ordenes = {};
    // nivel de cuenta: un poco más de oro inicial (nunca en partidas contra personas)
    if (!red) { const extra = FWM.heroe.oroInicialCuenta(); if (extra) App.estado.jugadores[miIndice].hucha.oro = (App.estado.jugadores[miIndice].hucha.oro || 0) + extra; }
    FWM.batallas.despues(App.estado, op.batalla || (op.tipo === "resistir" ? "resistir" : "conquista"), op);
    // Jefe de campaña (9 sep 2026): el rival 1 del último capítulo es Alvar el Cuervo, con héroe de leyenda,
    // más oro y su propio nombre. No hace falta una unidad nueva: un héroe al máximo ya pega bastante.
    if (op.jefe && App.estado.jugadores[1]) {
      const j = App.estado.jugadores[1];
      j.apodo = op.jefe.apodo || j.apodo;
      if (op.jefe.reino) j.nombre = op.jefe.reino;
      if (op.jefe.color) j.color = op.jefe.color;
      j.personalidad = "implacable";
      j.hucha.oro = Math.round((j.hucha.oro || 0) * (op.jefe.oro || 1.5));
      j.esJefe = true;
    }
    // personalidad y ayudas de cada máquina según la dificultad (fácil: menos oro; difícil: más oro y algo por turno)
    FWM.cerebro.prepararRivales(App.estado, op.dificultad || "normal", op.semilla);
    ...
    FWM.nube.evento("empieza", { tipo: op.tipo, rivales: n - 1, nivel: miHeroe.nivel, faccion: miFaccion, partidas: FWM.guardado.records().partidas || 0 }); App.pila = []; App.registro = []; App.sel = {}; App.posibles = vacio(); App.modo = null; App.avisoFin = 0; App.avisoTec = 0;
    ...
    // en las batallas sin pueblos no hay capital: se encuadra en el ejército y el aviso no habla de capital (21 sep 2026)
    const hexCapital = App.estado.jugadores[App.humano].capital;
    const cap = hexCapital ? App.estado.asentamientos[hexCapital] : null;
    const miTropa = FWM.estado.tropasDe(App.estado, App.humano)[0];
    const donde = hexCapital || (miTropa && FWM.estado.posicionTropa(App.estado, miTropa)) || null;
    App.sel = donde ? { hex: donde } : {}; App.refrescar();
    setTimeout(() => { App.L.encuadrar(App.estado, donde || App.miCapital()); App.dibujar(); }, 60);
    const color = App.humano === 1 ? (op.coop ? T.colorVerde : T.colorRojo) : T.colorAzul;
    const quien = T.tuReinoEs.replace("{reino}", App.estado.jugadores[App.humano].nombre).replace("{color}", color);
    FWM.paneles.aviso(cap ? quien.replace("{capital}", cap.nombre) : quien.replace(/\.?\s*Tu capital: \{capital\}\.?/, "").replace("{capital}", ""), 0); // se queda hasta el primer toque
    if (op.duelo) FWM.duelo.empezar(App, op.duelo, true); // con cara a cara y "preparado" antes de arrancar
    if (op.coop) FWM.duelo.empezar(App, op.coop, true);  // dobles: mismo arranque, con la pantalla de los dos
    // primera partida de verdad (sin tutorial): tres globos, uno por vez, y menos botones
    if (App.primeraPartida) {
      // sin pueblo (campo, asalto) el primer globo no puede hablar de "tu capital"
      const g = cap ? T.globos : [T.globoSinPueblo || T.globos[0]].concat(T.globos.slice(1)); let i = 0;
  ...
  App.cargarPartida = function (g) {
    ...
    const red = g.opciones && (g.opciones.duelo || g.opciones.coop);
    App.estado = g.estado; App.opciones = g.opciones; App.humano = red && red.rol === "invitado" ? 1 : 0; App.accionesTurno = [];
    if (red && App.estado.ganador == null) FWM.duelo.empezar(App, red);
  ...
  App.aplicarPaleta = function () {
    const c = paleta();
    const coop = !!(App.opciones && App.opciones.coop);
    if (App.estado) { App.estado.jugadores.forEach((j, i) => { if (j.horda) return; j.color = coop && i === 1 ? c[2] : c[i % c.length]; }); App.guardar(); App.refrescar(); }
  };
  ...
  App.finTurno = function (forzado) {
    ...
    App.anotar(r.eventos);
    App.refrescar();
    if (App.opciones && (App.opciones.duelo || App.opciones.coop)) { FWM.duelo.enviarTurno(acciones, App.estado); FWM.duelo.turnoCambia(); }
    App.jugarIAs(r.eventos);
  };

  // El capítulo de campaña que se está jugando (o null).
  App.capituloActual = function () {
    const o = App.opciones || {}; const c = o.campana && FWM.datosBase.campanas && FWM.datosBase.campanas[o.campana];
    return (c && (c.capitulos || []).find(x => x.id === o.capitulo)) || null;
  };
  // Botón "Objetivo" (25 sep 2026, Rodrigo: si te olvidas de lo que había que hacer, no había dónde mirarlo).
  // En campaña, la misma caja de la ficha del capítulo; fuera, cómo se gana esta partida.
  App.abrirObjetivo = function () {
    if (!App.estado || !App.datos) return;
    const T = App.datos.textos, TC = T.campana, e = App.estado; const cap = App.capituloActual();
    const esc = (x) => String(x == null ? "" : x).replace(/[&<>"]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[c]);
    let html;
    if (cap && cap.meta) {
      const dif = cap.dificultad === "dificil" ? T.dificil : cap.dificultad === "facil" ? T.facil : T.normal;
      html = `<h2>${esc(cap.nombre)}</h2>
        <div class="campana-meta"><b>🎯 ${TC.queHacer}</b><p>${esc(cap.meta.replace(/\{t\}/g, cap.limite))}</p></div>
        <p class="campana-datos"><b>${TC.contra}:</b> ${esc(cap.contra || "")} · ${dif}</p>
        ${cap.consejo ? `<p class="campana-consejo">💡 <b>${TC.consejo}:</b> ${esc(cap.consejo)}</p>` : ""}`;
    } else {
      const obj = FWM.paneles.textoObjetivo(App, e, App.datos) || "";
      const fin = e.limiteTurnos ? T.finPuntosPista.replace("{n}", e.limiteTurnos) : T.finEliminacionPista;
      html = `<h2>${T.objetivoBoton}</h2><div class="campana-meta"><b>🎯 ${TC.queHacer}</b><p>${esc(obj || fin)}</p></div>${obj && !e.oleadas ? `<p class="campana-datos">${esc(fin)}</p>` : ""}`;
    }
    html += `<p class="campana-datos">${T.turno} ${e.limiteTurnos ? Math.min(e.turno, e.limiteTurnos) + "/" + e.limiteTurnos : e.turno}</p>`;
    App.modal(html, [[T.cerrar || "OK", () => App.cerrarModal(), "btn btn-primario"]]);
  };

  // El texto de un aviso del guion: el del capítulo que se juega (campaña) o el general.
  App.textoGuion = function (clave) {
    const cap = App.capituloActual();
    return (cap && cap.avisos && cap.avisos[clave]) || App.datos.textos.guion[clave] || "";
  };
  App.guardarAvisoGuion = function (ev) {
    const t = ev.tipo === "avisoGuion" ? App.textoGuion(ev.clave) : ev.tipo === "objetivo" ? App.datos.textos.guion.objetivoNuevo : "";
    if (t) (App.avisosGuion = App.avisosGuion || []).push(t);
  };

  App.jugarIAs = function (previos) {
    App.ocupado = true; App.refrescar();
    // lo que pasó al acabar tu turno (el guion se revisa al acabar cada turno) también se cuenta
    App.ultimosEventos = (previos || []).slice();
    const paso = () => {
      const e = App.estado;
      if (e.ganador != null || e.jugadorActivo === App.humano || e.jugadores[e.jugadorActivo].humano || e.jugadores[e.jugadorActivo].remoto) {
        ...
        for (const ev of (App.ultimosEventos || [])) App.guardarAvisoGuion(ev);
        // lo que ha pasado en la historia (refuerzos, alguien que se va...) se cuenta al volver tu turno y se queda
        // en pantalla hasta que tocas: antes duraba dos segundos mientras jugaban las máquinas y no daba tiempo a leerlo
        if (App.avisosGuion && App.avisosGuion.length && e.ganador == null) { FWM.paneles.aviso(App.avisosGuion.join("\n\n"), 0); App.avisosGuion = []; }
        if (App.opciones && (App.opciones.duelo || App.opciones.coop)) { FWM.duelo.turnoCambia(); if (e.ganador == null && e.jugadorActivo !== App.humano) FWM.paneles.aviso(App.datos.textos.duelo.turnoDe.replace("{nombre}", e.jugadores[e.jugadorActivo].apodo || ""), 2500); }
        App.comprobarFin(App.ultimosEventos || []);
        return;
      }
      // avisos del guion (refuerzos, traiciones, objetivo nuevo): se cuentan en grande (21 sep 2026)
      if (App.ultimosEventos) for (const ev of App.ultimosEventos.splice(0)) App.guardarAvisoGuion(ev);
      // modo Resistir: antes de que jueguen las hordas, entra la oleada de esta ronda
      const esHorda = !!e.jugadores[e.jugadorActivo].horda;
      if (esHorda) {
        const nuevas = FWM.oleadas.oleada(e, App.datos);
        if (nuevas.length) { FWM.sonido.tambor(); FWM.paneles.aviso(App.datos.textos.resistir.llegaOleada.replace("{n}", nuevas.length).replace("{ronda}", e.turno), 2600); }
        App.refrescar();
      }
      // el tutorial sigue con la fácil de antes: la primera partida tiene que ser un paseo
      const ia = esHorda ? FWM.ias.hordas : App.tutorial ? FWM.ia.jugar : (FWM.ias[(App.opciones && App.opciones.dificultad) || "normal"] || FWM.ias.normal);
  ...
  App.reproducirPasos = function (pasos, fin) {
    ...
    const anotables = ["ataque", "muere", "conquista", "funda", "castillo", "ciudad", "investigado", "bancarrota", "brecha", "eliminado", "victoria", "turno", "asedio", "asciende", "refuerzos", "cambiaBando", "retirada", "objetivo", "avisoGuion"];
    const aplicarPaso = (p) => {
  ...
  App.avanzarOrden = function (id) {
    ...
    const enemigoCerca = FWM.hex.vecinos(pos).some(v => { const x = FWM.estado.tropaEn(e, v); return x && FWM.estado.enemigos(e, x.dueno, t.dueno); });
  ...
  App.comprobarFin = function (eventos) {
    ...
    const faccionJugada = (e.jugadores[App.humano] && e.jugadores[App.humano].faccion) || FWM.heroe.faccion();
    const heroeAntes = FWM.heroe.progreso(faccionJugada);
    if (!e.resultadoAnotado) {
      ...
      const resultado = { gano: FWM.victoria.haGanado(e, App.humano), turnos: e.turno, tipo: (App.opciones && App.opciones.tipo) || "normal", mapa: e.mapa.nombre, semilla: e.semilla, fecha: new Date().toISOString().slice(0, 10), puntos: FWM.victoria.puntos(e, App.datos, App.humano),
        bando: jH.bando, faccion: jH.faccion || null, matadas: stH.matadas || 0, perdidas: stH.perdidas || 0, conquistas: stH.conquistas || 0, puntosClave, heroesMatados: stH.heroesMatados || 0, rivales: e.jugadores.length - 1,
        ...
        abandonoRival: e.finDuelo === "abandono", dificultad: (App.opciones && App.opciones.dificultad) || "normal", capitulo: App.opciones && App.opciones.capitulo, campana: App.opciones && App.opciones.campana, batalla: App.opciones && App.opciones.batalla,
        rondas: e.oleadas ? FWM.oleadas.rondas(e) : 0, coop: !!(App.opciones && App.opciones.coop) };
      // en Resistir siempre acabas cayendo: los puntos del héroe salen de las rondas aguantadas, no del reino
      // en dobles los puntos se reparten entre los dos: aguantar acompañado cuesta menos
      if (e.horda) resultado.puntos = Math.round(resultado.rondas * 20 / (resultado.coop ? 2 : 1));
      ...
      // los puntos suben la escalera de la facción con la que se ha jugado (al doble si va por detrás de tu mejor facción)
      App.subidaFaccion = FWM.heroe.anotarPartida(faccionJugada, resultado.puntos);
      ...
      try { App.recompensasHeroe.cuenta = FWM.heroe.cobrarCuenta(); } catch (x) { App.recompensasHeroe.cuenta = null; }
      ...
      if (resultado.tipo === "campana" && resultado.gano && FWM.campana) { const pr = FWM.campana.de(resultado.campana).superar(resultado.capitulo, resultado.turnos); if (pr) { App.recompensasHeroe.campana = Object.assign({ capitulo: resultado.capitulo }, pr); App.recompensasHeroe.oro += pr.oro || 0; } }
      ...
      // en Resistir la puntuación del ranking son las rondas aguantadas, no los puntos de reino
      const envio = { puntos: e.oleadas ? FWM.oleadas.rondas(e) : FWM.victoria.puntos(e, App.datos, App.humano), tipo: ((App.opciones && App.opciones.coop) ? "resistir_coop" : (App.opciones && App.opciones.tipo) || "normal"), turnos: e.turno, gano: FWM.victoria.haGanado(e, App.humano), semilla: e.semilla, mapa: e.mapa.nombre, faccion: faccionJugada };
    ...
    if (primeraVez) {
      const heroeDespues = FWM.heroe.progreso(faccionJugada);
      const subida = { faccion: faccionJugada, puntos: Math.max(0, heroeDespues.puntosMejora - heroeAntes.puntosMejora), nivel: heroeDespues.nivel > heroeAntes.nivel ? heroeDespues.nivel : null, disponibles: heroeDespues.disponibles, doble: !!(App.subidaFaccion && App.subidaFaccion.doble), sumados: App.subidaFaccion ? App.subidaFaccion.sumados : 0 };
  ...
  App.colocarSenal = function () {
    ...
    if (s.sel) { const n = document.querySelector(s.sel); if (n && !n.hidden && n.offsetWidth) { const r = n.getBoundingClientRect(); x = r.left + r.width / 2; y = r.top + r.height / 2; } }
      ...
      // el punto puede ser una función: así se recalcula al mover el mapa y la flecha sigue señalando
      // lo mismo (antes era un píxel fijo y al arrastrar apuntaba a cualquier sitio)
      const p = (typeof s.punto === "function" ? s.punto() : s.punto) || App.L.centro(s.hex);
    ...
    // roja con filo blanco: el dorado era el mismo color que la selección del asentamiento y se confundían
    el.innerHTML = `<svg viewBox="0 0 24 34" width="30" height="42" aria-hidden="true"><path d="M12 2v22" stroke="#fff" stroke-width="8" stroke-linecap="round" fill="none"/><path d="M12 33l-10.5-14h21z" fill="#fff"/><path d="M12 3v21" stroke="#e0342a" stroke-width="5" stroke-linecap="round" fill="none"/><path d="M12 31l-8.5-11h17z" fill="#e0342a"/></svg>`;
  ...
  App.animarAccion = function (antes, despues, accion, eventos) {
    ...
    if ((accion.tipo === "atacar" || accion.tipo === "asediar") && t && origen) {
      ...
      for (const e of eventos || []) {
        ...
        if (e.tipo === "muere" && e.hex) {
          ...
          App.anim.fantasmas.push({ t0: ahora + cuando, dur: 500, hex: e.hex, tipo: e.tropa, color: antes.jugadores[e.jugador].color, enemigo: e.jugador !== App.humano, heroe: antes.jugadores[e.jugador].heroe, faccion: antes.jugadores[e.jugador].faccion });
    ...
    desertan.forEach((e, i) => {
      App.anim.fantasmas.push({ t0: ahora + i * 220, dur: 700, hex: e.hex, tipo: e.tropa, color: antes.jugadores[e.jugador].color, enemigo: e.jugador !== App.humano, faccion: antes.jugadores[e.jugador].faccion });
  ...
  App.etiquetasBotones = function () {
    ...
    poner("btn-objetivo", "🎯 " + T.objetivoBoton, "🎯 " + T.objetivoBoton); // con la palabra también en el móvil: solo la diana no se entendía
  ...
  const PRESETS = {
    ...
    rapida: { tecnologia: "todo", hucha: 2, limite: 15 },
    dia: { tecnologia: "todo", hucha: 2, limite: 15 }, // mapa del día: como la Rápida, con la semilla del día
    normal: { tamano: "mediano", tecnologia: "arbol", hucha: 1, limite: 0 },
    duelo: { tamano: "pequeno", tecnologia: "todo", hucha: 2, limite: 20, rivales: 1 }, // 1 contra 1 en directo
    escenario: { tecnologia: "todo", hucha: 2, limite: 60 },
    resistir: { mapaHecho: "arena", tecnologia: "todo", hucha: 2, limite: 0, rivales: 1 }, // resistir hordas: sin límite de turnos // grandes batallas: el mapa y los turnos los pone el escenario
  };

  // Opciones de una partida Rápida con lo que se puede suponer (para Jugar sin preguntar nada).
  App.opcionesRapida = function () { return Object.assign({ tipo: "rapida", rivales: 2, dificultad: "normal", faccion: FWM.heroe.faccion(), semilla: Math.floor(Math.random() * 1e6) + 1 }, PRESETS.rapida); };
  ...
  App.formularioPartida = function () {
    ...
    const op = Object.assign({ rivales: 2, dificultad: "normal", tipo: "rapida", tecnologia: "todo", hucha: 2, limite: 15 }, App.opciones || {});
    ...
    // la facción: solo las que tienes (13 sep 2026: ya no se elige bando; el rasgo lo pone la facción)
    const propias = FWM.facciones.lista().filter(f => FWM.heroe.tiene(f));
    const facActual = propias.includes(FWM.heroe.faccion()) ? FWM.heroe.faccion() : propias[0];
    cont.innerHTML = `
      <div class="campo"><label>${T.facciones.juegasCon}</label>${sel("op-faccion", propias.map(f => [f, App.datos.facciones[f].nombre]), facActual)}</div>
      <p class="pista" id="op-rasgo"></p>
      <div class="campo"><label>${T.batallas.tipo}</label>${sel("op-batalla", FWM.batallas.lista().map(id => [id, T.batallas.tipos[id].nombre]), op.batalla || "conquista")}</div>
      <p class="pista" id="op-batalla-pista"></p>
      <div class="campo"><label>${T.jugadores}</label>${sel("op-rivales", [1, 2, 3, 4].map(n => [String(n), String(n)]), String(op.rivales))}</div>
      <div class="campo" id="op-equipos-campo"><label>${T.batallas.equipos}</label><div id="op-equipos" class="op-equipos"></div></div>
    ...
    const pintarRasgo = () => { const f = App.datos.facciones[$("op-faccion").value]; $("op-rasgo").textContent = f ? `${f.lider} · ${f.rasgo.nombre}: ${f.rasgo.texto}` : ""; };
    $("op-faccion").addEventListener("change", () => { FWM.heroe.elegirFaccion($("op-faccion").value); pintarRasgo(); }); pintarRasgo();
    // equipos: una fila por jugador (tú y las máquinas); dos en el mismo equipo juegan juntos
    const equipos = [];
    const pintarEquipos = () => {
      const n = parseInt($("op-rivales").value, 10) + 1;
      const caja = $("op-equipos"); caja.innerHTML = "";
      for (let i = 0; i < n; i++) {
        if (equipos[i] == null) equipos[i] = i;
        const fila = document.createElement("div"); fila.className = "op-equipo";
        const quien = i === 0 ? T.batallas.tu : T.batallas.maquina + " " + i;
        fila.innerHTML = `<span>${quien}</span>`;
        const s = document.createElement("select");
        for (let e = 0; e < n; e++) { const o = document.createElement("option"); o.value = String(e); o.textContent = T.batallas.equipo.replace("{n}", "ABCD"[e] || e + 1); if (equipos[i] === e) o.selected = true; s.appendChild(o); }
        s.addEventListener("change", () => { equipos[i] = parseInt(s.value, 10); });
        fila.appendChild(s); caja.appendChild(fila);
      }
      equipos.length = n;
    };
    const pintarBatalla = () => {
      const id = $("op-batalla").value; const t = T.batallas.tipos[id];
      $("op-batalla-pista").textContent = t ? t.pista : "";
      const d = FWM.batallas.def(id);
      // en Resistir mandan la arena y las hordas: ni equipos ni fin de partida
      $("op-equipos-campo").hidden = id === "resistir";
      $("op-rivales").closest(".campo").hidden = id === "resistir";
      $("op-fin").closest(".campo").hidden = !!d.objetivo || id === "resistir";
      pintarFin();
    };
    $("op-rivales").addEventListener("change", pintarEquipos);
    ...
    $("op-batalla").addEventListener("change", pintarBatalla); pintarEquipos(); pintarBatalla();
    const leer = () => {
      const batalla = $("op-batalla").value; const d = FWM.batallas.def(batalla);
      const resistir = batalla === "resistir";
      const base = Object.assign({ tipo: resistir ? "resistir" : "rapida", batalla, rivales: resistir ? 1 : parseInt($("op-rivales").value, 10),
        dificultad: $("op-dificultad").value, faccion: $("op-faccion").value, semilla: Math.floor(Math.random() * 1e6) + 1 }, PRESETS.rapida);
      if (resistir) return Object.assign(base, { mapaHecho: "arena", limite: 0 });
      base.equipos = equipos.slice(0, base.rivales + 1);
      base.limite = d.objetivo ? (d.limite || 0) : ($("op-fin").value === "puntos" ? parseInt($("op-turnos").value, 10) : 0);
      return base;
    };
