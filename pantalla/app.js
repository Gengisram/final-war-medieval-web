...
const App = (function () {
  ...
  App.nuevaPartida = function (op) {
    ...
    for (let i = 1; i < n; i++) {
      ...
      const b = libres.length ? libres.shift() : gb.elegir(jugables);
      // el jefe de campaña lleva un héroe de leyenda con todos los puntos gastados, no uno de mi nivel
      const esJefe = op.jefe && i === 1;
      const hIA = esJefe ? FWM.heroe.paraIA(op.semilla + 777, op.jefe.nivel || 8, 40, op.jefe.clase) : FWM.heroe.paraIA(op.semilla + 1000 * i, miHeroe.nivel, gastados);
      jugadores.push({ nombre: nombreDe(b), color: COLORES[i % COLORES.length], humano: false, bando: b, apodo: apodos[i - 1] || ("IA " + i), heroe: hIA });
    }
    ...
    let miIndice = 0;
    // Bárbaros a dobles: dos humanos (0 anfitrión, 1 invitado) y las hordas de últimas. Igual que el
    // duelo, los dos aparatos llevan el mismo motor y se mandan las acciones (9 sep 2026).
    if (op.coop) {
      const d = op.coop; miIndice = d.rol === "invitado" ? 1 : 0; const otro = 1 - miIndice;
      const mio = FWM.nube.nombre() || FWM.guardado.ajustes().nombre || App.datos.textos.fichas.tuMayus;
      const hCompi = d.rival.heroe || FWM.heroe.paraIA(op.semilla + 4242, miHeroe.nivel, gastados);
      jugadores[miIndice] = Object.assign(jugadores[miIndice], { humano: true, remoto: false, apodo: mio, heroe: miHeroe });
      jugadores[otro] = Object.assign(jugadores[otro], { humano: false, remoto: d.rol !== "bot", apodo: d.rival.nombre, heroe: hCompi });
    }
    if (op.duelo) {
  ...
  App.comprobarFin = function (eventos) {
    ...
    if (!e.resultadoAnotado) {
      ...
      const envio = { puntos: e.barbaros ? FWM.barbaros.rondas(e) : FWM.victoria.puntos(e, App.datos, App.humano), tipo: ((App.opciones && App.opciones.coop) ? "barbaros_coop" : (App.opciones && App.opciones.tipo) || "normal"), turnos: e.turno, gano: e.ganador === App.humano, semilla: e.semilla, mapa: e.mapa.nombre };
