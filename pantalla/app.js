...
const App = (function () {
  ...
  App.nuevaPartida = function (op) {
    ...
    const jugadores = [{ nombre: nombreDe(App.datos.facciones[miFaccion].nombre), color: COLORES[0], humano: true, faccion: miFaccion, nivel: miHeroe.nivel, heroe: miHeroe, tierra: tierraHumano }];
    ...
    for (let i = 1; i < n; i++) {
      // en Bárbaros la horda es SIEMPRE el último jugador; en dobles, el 1 es el compañero humano
      if (op.tipo === "barbaros" && i === n - 1) { jugadores.push({ nombre: App.datos.bandos.barbaros.nombre, color: "#6b6455", humano: false, bando: "barbaros", apodo: App.datos.bandos.barbaros.nombre }); continue; }
      ...
      const nivelIA = esJefe ? 10 : miHeroe.nivel;
      ...
      if (enemigo && enemigo.bando && !enemigo.faccion && App.datos.bandos[enemigo.bando]) {
        ...
        jugadores.push({ nombre: nombreDe(enemigo.reino || (jf && jf.reino) || App.datos.bandos[enemigo.bando].nombre), color: enemigo.color || COLORES[i % COLORES.length], humano: false, bando: enemigo.bando, nivel: nivelIA, apodo: enemigo.apodo || apodoDe(enemigo.cultura || enemigo.bando, i), heroe: hIA });
        continue;
      }
      ...
      jugadores.push({ nombre: nombreDe((enemigo && enemigo.reino) || App.datos.facciones[f].nombre), color: (enemigo && enemigo.color) || COLORES[i % COLORES.length], humano: false, faccion: f, nivel: nivelIA, apodo: (enemigo && enemigo.apodo) || apodoDe((enemigo && enemigo.cultura) || f, i), heroe: hIA });
    }
    ...
    let miIndice = 0;
    // Bárbaros a dobles y duelos: cada uno lleva SU facción (viene en la sesión). El anfitrión es el 0.
    ...
    FWM.paneles.aviso(T.tuReinoEs.replace("{reino}", App.estado.jugadores[App.humano].nombre).replace("{color}", App.humano === 1 ? (op.coop ? T.colorVerde : T.colorRojo) : T.colorAzul).replace("{capital}", cap.nombre), 0); // se queda hasta el primer toque
  ...
  App.aplicarPaleta = function () {
    ...
    if (App.estado) { App.estado.jugadores.forEach((j, i) => { if (j.barbaros) return; j.color = coop && i === 1 ? c[2] : c[i % c.length]; }); App.guardar(); App.refrescar(); }
  };
  ...
  App.jugarIAs = function () {
    ...
    const paso = () => {
      ...
      const ia = esHorda ? FWM.ias.barbaros : App.tutorial ? FWM.ia.jugar : (FWM.ias[(App.opciones && App.opciones.dificultad) || "normal"] || FWM.ias.normal);
  ...
  App.comprobarFin = function (eventos) {
    ...
    if (!e.resultadoAnotado) {
      ...
      const resultado = { gano: e.ganador === App.humano, turnos: e.turno, tipo: (App.opciones && App.opciones.tipo) || "normal", mapa: e.mapa.nombre, semilla: e.semilla, fecha: new Date().toISOString().slice(0, 10), puntos: FWM.victoria.puntos(e, App.datos, App.humano),
        ...
        rondas: e.barbaros ? FWM.barbaros.rondas(e) : 0, coop: !!(App.opciones && App.opciones.coop) };
      ...
      if (e.barbaros) resultado.puntos = Math.round(resultado.rondas * 20 / (resultado.coop ? 2 : 1));
      ...
      const envio = { puntos: e.barbaros ? FWM.barbaros.rondas(e) : FWM.victoria.puntos(e, App.datos, App.humano), tipo: ((App.opciones && App.opciones.coop) ? "barbaros_coop" : (App.opciones && App.opciones.tipo) || "normal"), turnos: e.turno, gano: e.ganador === App.humano, semilla: e.semilla, mapa: e.mapa.nombre, faccion: faccionJugada };
  ...
  App.formularioPartida = function () {
    ...
    const op = Object.assign({ rivales: 2, dificultad: "normal", tipo: "rapida", tecnologia: "todo", hucha: 2, recursos: "equilibrado", limite: 15 }, App.opciones || {});
    ...
    const leer = () => Object.assign({ tipo: "rapida", rivales: parseInt($("op-rivales").value, 10), dificultad: $("op-dificultad").value, faccion: $("op-faccion").value, semilla: Math.floor(Math.random() * 1e6) + 1 },
