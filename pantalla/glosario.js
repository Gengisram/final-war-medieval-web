...
FWM.glosario = (function () {
  const PESTANAS = ["facciones", "bandos", "tropas", "asentamientos", "terrenos", "reglas"];
  ...
  function fichas(App) {
    ...
    // las seis facciones (13 sep 2026)
    for (const id of FWM.facciones.lista()) {
      const f = datos.facciones[id]; const TF = T.facciones; const u = datos.tropas[f.unidad];
      const filas = [[TF.lider, `${f.lider} (${datos.heroes.clases[f.clase].nombre})`], [TF.estilo, f.estilo], [TF.rasgo, `${f.rasgo.nombre}: ${f.rasgo.texto}`], [TF.poder, `${f.poder.nombre}: ${f.poder.texto}`],
        [TF.unidad, u.nombre], [TF.noTiene, f.sinTropas.map(t => datos.tropas[t].nombre).join(", ")], [TF.campana, FWM.datosBase.campanas[id] ? FWM.datosBase.campanas[id].nombre : "—"], ["", f.pago ? (FWM.cobro ? FWM.cobro.precio(id) : "") : TF.gratis]];
      lista.push({ pestana: "facciones", id, nombre: f.nombre, descripcion: f.descripcion, filas, icono: () => { const d = document.createElement("span"); d.innerHTML = FWM.emblemas ? FWM.emblemas.svg(id, 48) : ""; return d; }, clave: normalizar(f.nombre + " " + f.lider + " " + f.rasgo.nombre + " " + f.poder.nombre + " " + u.nombre) });
    }
    ...
    for (const [id, t] of Object.entries(datos.tropas)) {
      ...
      const filas = [
        ...
        [T.alcance, t.stats.alcance === 0 ? F.cuerpoACuerpo : t.stats.alcance], [T.movimiento, (FWM.stats.casillas(datos, t.stats.movimiento) === 1 ? F.casilla1 : F.casillasN).replace("{n}", FWM.stats.casillas(datos, t.stats.movimiento))],
      ...
      if (t.faccion && datos.facciones[t.faccion]) {
        const TF = T.facciones; filas.unshift([T.facciones.titulo, datos.facciones[t.faccion].nombre]);
        const base = FWM.paneles.textoHabilidades(datos, { firme: t.firme, furia: t.furia }); if (base) filas.push([F.especial, base]);
        filas.push([TF.curtida, FWM.paneles.textoHabilidades(datos, t.curtida)], [TF.maestra, FWM.paneles.textoHabilidades(datos, t.maestra)]);
      } else if (FWM.facciones.nivelTropa(id) > 1) filas.push([T.heroeUI.nivel, String(FWM.facciones.nivelTropa(id)) + " · " + FWM.datosBase.escalera[FWM.facciones.nivelTropa(id) - 1].nombre]);
    ...
    for (const [id, te] of Object.entries(datos.terrenos)) {
      // en palabras del tablero: el llano se cruza; bosque y colina te paran al entrar
      const filas = [[F.mover, te.costeMovimiento == null ? F.noSePuede : te.costeMovimiento > (datos.reglas.puntosPorPaso || 2) ? F.teParas : F.unaCasilla]];
