...
FWM.figuras = (function () {
  ...
  // Mongoles: el Kan a caballo, con gorro de piel alto, deel con ribete dorado y arco a la espalda.
  F.heroe_kan = (ctx, x, y, s, op) => {
    op = op || {};
    const conMontura = op.heroe && op.heroe.objetos && op.heroe.objetos.montura;
    heroeBase(ctx, x, y, s, op, (marcas) => {
      // arco a la espalda
      ctx.strokeStyle = "#5a3a1e"; ctx.lineWidth = Math.max(2, s * .09);
    ...
    }, conMontura ? null : () => caballo(ctx, x - s * .05, y + s * .12, s * .95, "#9a6b3a", op.color || "#888", op));
  };
