...
FWM.tutorial = (function () {
  ...
  const PASOS = () => [
    ...
    { id: "reclutar",
      destacar: () => capital(),
      senal: () => ({ punto: () => {
        if (!App.reclutaAbierto) return puntoReclutar(capital());
        const col = document.getElementById("recluta"); const b = col && !col.hidden && col.querySelector("button");
        if (!b) return puntoReclutar(capital());
        const r = b.getBoundingClientRect(); const cv = document.getElementById("mapa").getBoundingClientRect();
        return { x: r.left + r.width / 2 - cv.left, y: r.top + r.height / 2 - cv.top };
      } }),
  ...
  function empezar(app) {
    ...
    App.nuevaPartida({ tipo: "rapida", rivales: 1, dificultad: "facil", tamano: "pequeno", tecnologia: "todo", hucha: 2, recursos: "equilibrado", limite: 15, faccion: FWM.heroe.faccion(), semilla: 4242 }); // con tu facción: lo que ganas va a la tuya (antes, siempre a Castilla)
    App.tutorial = true;
    pintar();
  }
  ...
  function terminar() {
    ...
    if (FWM.nube && FWM.nube.evento) FWM.nube.evento("tutorial", { acabado: true });
