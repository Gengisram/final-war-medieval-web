...
FWM.resultado = (function () {
  function render(App, opciones) {
    ...
    const gano = FWM.victoria.haGanado(estado, App.humano);
    ...
    const fallido = (estado.registro || []).some(ev => ev.tipo === "victoria" && ev.objetivoFallido);
    const sinCapital = (estado.registro || []).some(ev => ev.tipo === "victoria" && ev.capitalPerdida);
    ...
    // en Resistir siempre acabas cayendo: lo que cuenta son las rondas, no quién gana
    const esResistir = !!estado.oleadas;
    const rondasB = esResistir ? FWM.oleadas.rondas(estado) : 0;
    const titulo = esResistir ? (App.opciones && App.opciones.coop ? T.coop.aguantasteis : T.resistir.aguantaste).replace("{n}", rondasB)
      : anulada ? T.duelo.anulada : (gano ? T.ganaste : (estado.ganador != null && estado.jugadores[estado.ganador] ? T.victoriaPara.replace("{nombre}", FWM.paneles.nombreReino(App, estado.jugadores[estado.ganador]).replace(/^(<span[^>]*>)?(Los|Las|La|El|The) /, (m) => m.replace(/(Los|Las|La|El|The) $/, (x) => x.toLowerCase()))) : T.perdiste)) + (porPuntos ? `<small class="suave" style="display:block">${T.victoriaPorPuntos}</small>` : fallido ? `<small class="suave" style="display:block">${T.objetivoFallido}</small>` : sinCapital ? `<small class="suave" style="display:block">${T.capitalPerdida}</small>` : "") + motivoDuelo;
    cont.innerHTML = `<h2>${titulo}</h2><p class="suave">${nombrePartida(App, estado)} · ${T.turno.toLowerCase()} ${turnoVisible(estado)} · ${T.tipos[(App.opciones && App.opciones.tipo) || "normal"] || ""}</p>`;
    ...
    for (const j of (esResistir ? estado.jugadores.filter(x => !x.horda) : estado.jugadores)) {
      const s = st[j.id] || {};
      ...
      tr.innerHTML = `<td class="izq"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${j.color};margin-right:6px"></span>${nombre} ${estadoJ}</td>
        <td class="num" title="${FWM.victoria.desglose(estado, datos, j.id).map(d => d.texto + ": " + d.puntos).join("\n")}" style="cursor:help"><b>${esResistir ? Math.round(rondasB * 20 / (App.opciones && App.opciones.coop ? 2 : 1)) : FWM.victoria.puntos(estado, datos, j.id)}</b> <small class="suave">?</small></td><td class="num">${FWM.estado.asentamientosDe(estado, j.id).length}</td><td class="num">${FWM.estado.tropasDe(estado, j.id).length}</td>
    ...
    const misPuntos = esResistir ? Math.round(rondasB * 20 / (App.opciones && App.opciones.coop ? 2 : 1)) : FWM.victoria.puntos(estado, datos, App.humano);
    ...
    // 14 sep 2026: en duelos y dobles "Revancha" y "Otra partida" reabrían una sesión de red ya cerrada; en la
    // campaña, "Otra partida" repetía el capítulo. Ahora cada modo ofrece lo que tiene sentido.
    const op = App.opciones || {};
    const enRed = !!(op.duelo || op.coop);
    const C = op.tipo === "campana" && op.campana && FWM.campana ? FWM.campana.de(op.campana) : null;
    if (C) {
      const irCampana = () => { App.cerrarModal(); FWM.inicio.mostrar(App); FWM.inicio.vistaCampana(op.campana); };
      if (FWM.victoria.haGanado(estado, App.humano) && C.siguiente()) fila.appendChild(App.boton(T.campana.seguir, () => { App.cerrarModal(); FWM.inicio.mostrar(App); FWM.inicio.vistaCampana(op.campana, C.siguiente().id); }, "btn btn-primario"));
      else { fila.appendChild(App.boton(T.campana.repetirCap, () => App.revancha(), "btn btn-primario")); fila.appendChild(App.boton(T.campana.volver, irCampana, "btn btn-claro")); }
    } else if (!enRed) {
  ...
  function posicionDe(App, estado, datos) {
    ...
    if (FWM.victoria.haGanado(estado, App.humano)) pos = 0; else if (estado.ganador != null && pos === 0) pos = 1;
    return pos;
  }
  // el turno que se enseña: al pasar el límite el motor ya va por el siguiente, pero la partida era de N turnos
  const turnoVisible = (e) => e.limiteTurnos ? Math.min(e.turno, e.limiteTurnos) : e.turno;
  // En campaña, el nombre del capítulo ("El molino del río"), no el del mapa por dentro ("La Hoz"), que no dice nada
  const nombrePartida = (App, e) => {
    const o = App.opciones || {}; const c = o.campana && FWM.datosBase.campanas && FWM.datosBase.campanas[o.campana];
    const cap = c && (c.capitulos || []).find(x => x.id === o.capitulo);
    return cap ? cap.nombre : (e.mapa.nombre || "");
  };
  ...
  function ceremonia(App, opciones, alAcabar) {
    const { estado, datos } = App; const T = datos.textos;
    const misPuntos = estado.oleadas ? Math.round(FWM.oleadas.rondas(estado) * 20 / (App.opciones && App.opciones.coop ? 2 : 1)) : FWM.victoria.puntos(estado, datos, App.humano);
    const pos = posicionDe(App, estado, datos);
    const primero = pos === 0;
    const r = opciones.records || {}; const totalAntes = Math.max(0, (r.puntosTotal || 0) - misPuntos);
    const nuevas = medallasNuevas(datos, opciones.nuevas);

    // los bocadillos de la partida no se ven a través de la ceremonia (14 sep 2026: el globo del tutorial y el
    // aviso de "Eres…" se mezclaban con el "¡Victoria!")
    for (const id of ["aviso", "tutorial"]) { const el = document.getElementById(id); if (el) el.hidden = true; }
    ...
    const esResistir = !!estado.oleadas;
    const rondas = esResistir ? FWM.oleadas.rondas(estado) : 0;
    const coop = !!(App.opciones && App.opciones.coop);
    const recordAntes = (opciones.records && opciones.records[(coop ? "resistirCoopRecord" : "resistirRecord") + "Antes"]) || 0;
    const hPos = document.createElement("div"); hPos.className = "ceremonia-pos";
    // con dos bandos, no ganar es perder: "Segundo" sonaba a podio (23 sep 2026)
    // y en campaña tampoco hay podio: o ganas el capítulo o no (24 sep 2026: salía "Segundo" en los de tres bandos)
    const dosBandos = FWM.victoria.equipos(estado).size <= 2 || !!(App.opciones && App.opciones.tipo === "campana");
    hPos.textContent = esResistir ? rondas + "" : (primero ? T.victoria : dosBandos ? T.derrota : (T.posiciones[pos] || (pos + 1) + ".º"));
    if (primero && !esResistir) hPos.classList.add("victoria"); // gana: la palabra, no el puesto
    caja.appendChild(hPos);
    if (esResistir) { const s2 = document.createElement("div"); s2.className = "ceremonia-sub"; s2.textContent = rondas > recordAntes ? T.resistir.nuevoRecord.replace("{n}", rondas) : (coop ? T.coop.aguantasteis : T.resistir.aguantaste).replace("{n}", rondas); caja.appendChild(s2); }
    const sub = document.createElement("div"); sub.className = "ceremonia-sub suave"; sub.textContent = `${nombrePartida(App, estado)} · ${T.turno.toLowerCase()} ${turnoVisible(estado)}`; caja.appendChild(sub);
    ...
    if (esResistir ? rondas > recordAntes && rondas > 0 : primero) { confeti(capa); FWM.sonido.victoria(); } else FWM.sonido.derrota();
    ...
    (async () => {
      ...
      if (sub.nivel) {
        await espera(500);
        // 13 sep 2026: el nivel es de la facción con la que se ha jugado, y se dice qué da (su hito o sus tropas)
        const f = sub.faccion || FWM.heroe.faccion();
        const hp = FWM.heroe.paraPartida({}, f); hp.nivel = sub.nivel; const dn = FWM.heroes.datosNivel(sub.nivel);
        const d = document.createElement("div"); d.className = "ceremonia-nivel";
        d.appendChild(FWM.figuras.canvasHeroe(hp, "#2f6fd6", 140, true));
        // "¡Has subido al nivel 2 con Castilla!" · "Tu nuevo rango: ESCUDERO" · y debajo, una línea por cada cosa
        // nueva, con su dibujo ("Nueva tropa: Monje · No pelea: cura."). Antes: "2 · ESCUDERO" y "Abre: Monje" (25 sep 2026)
        const t1 = document.createElement("div"); t1.className = "cer-nivel-txt"; t1.textContent = TH.subesNivel.replace("{n}", sub.nivel).replace("{faccion}", (datos.facciones[f] && datos.facciones[f].nombre) || ""); d.appendChild(t1);
        const t0 = document.createElement("div"); t0.className = "cer-nivel-rango"; t0.textContent = TH.tuRango; d.appendChild(t0);
        const t2 = document.createElement("div"); t2.className = "cer-nivel-nombre"; t2.textContent = dn.nombre.toUpperCase(); d.appendChild(t2);
        if (FWM.inicio && FWM.inicio.novedadesNivel) for (const nv of FWM.inicio.novedadesNivel(f, sub.nivel)) {
          const u = document.createElement("div"); u.className = "cer-nivel-unidad";
          if (nv.tropa) u.appendChild(FWM.iconos.canvasTropa(nv.tropa, "#2f6fd6", 48, false, f));
          const ut = document.createElement("span"); ut.innerHTML = nv.texto; u.appendChild(ut); d.appendChild(u);
        }
      ...
      if (sub.doble && sub.sumados) partes.push(`<b>×2</b> ${datos.facciones[sub.faccion] ? datos.facciones[sub.faccion].nombre : ""}`);
      if (rec.cuenta) { partes.push(T.cuentaNivel.sube.replace("{n}", FWM.heroe.nivelCuenta()).replace("{oro}", rec.cuenta.oro)); for (const a of rec.cuenta.aspectos || []) if (datos.objetos[a]) partes.push(T.cuentaNivel.aspecto.replace("{nombre}", datos.objetos[a].nombre)); }
  ...
  function confeti(capa) {
    ...
    // se remide en cada vuelta: si la ceremonia sale mientras la ventana aún no tiene tamaño (o se gira
    // el móvil), el lienzo se quedaba a cero y no se veía un solo papelillo
    let anchoV = 0, altoV = 0;
    const ajustar = () => { if (innerWidth === anchoV && innerHeight === altoV) return; anchoV = innerWidth; altoV = innerHeight; c.width = anchoV * dpr; c.height = altoV * dpr; }; ajustar();
    const colores = ["#e8b923", "#d63b3b", "#2f6fd6", "#2e9e4f", "#ff9f1a", "#f2e3c2"];
    // 9 sep 2026: Rodrigo pidió que ganar se celebre más. Tres veces más papelillos, dos cañones que
    // disparan hacia arriba desde las esquinas de abajo y una segunda tanda a los dos segundos.
    const N = 420, ps = [];
    const papel = (x, y, vx, vy, i) => ({ x, y, vx, vy, w: 6 + Math.random() * 8, h: 4 + Math.random() * 7, a: Math.random() * Math.PI * 2, va: (Math.random() - .5) * .3, color: colores[i % colores.length], fase: Math.random() * Math.PI * 2, g: 0.02 + Math.random() * 0.03 });
    for (let i = 0; i < N * .6; i++) ps.push(papel(Math.random() * innerWidth, -20 - Math.random() * innerHeight * .9, (Math.random() - .5) * 1.4, 1.5 + Math.random() * 3, i));
    const canon = (lado) => { for (let i = 0; i < N * .2; i++) ps.push(papel(lado ? innerWidth + 10 : -10, innerHeight + 10, (lado ? -1 : 1) * (3 + Math.random() * 5), -(7 + Math.random() * 7), i)); };
    canon(0); canon(1);
    setTimeout(() => { if (c.isConnected) { canon(0); canon(1); for (let i = 0; i < 80; i++) ps.push(papel(Math.random() * innerWidth, -20, (Math.random() - .5) * 1.4, 2 + Math.random() * 3, i)); } }, 2000);
    ...
    const paso = () => {
      ...
      for (const p of ps) {
        p.x += p.vx + Math.sin(t * 3 + p.fase) * .6; p.y += p.vy; p.a += p.va; p.vy += p.g; // los cañonazos caen
        if (p.y > innerHeight + 20 && t < 6) { p.y = -20; p.x = Math.random() * innerWidth; p.vy = 1.5 + Math.random() * 2.5; }
      ...
      if (t < 10) requestAnimationFrame(paso); else c.remove();
    };
    requestAnimationFrame(paso);
  }
