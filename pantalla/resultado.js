...
FWM.resultado = (function () {
  function render(App, opciones) {
    ...
    const titulo = esBarbaros ? (App.opciones && App.opciones.coop ? T.coop.aguantasteis : T.barbaros.aguantaste).replace("{n}", rondasB)
    ...
    for (const j of (esBarbaros ? estado.jugadores.filter(x => !x.barbaros) : estado.jugadores)) {
      ...
      tr.innerHTML = `<td class="izq"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${j.color};margin-right:6px"></span>${nombre} ${estadoJ}</td>
        <td class="num" title="${FWM.victoria.desglose(estado, datos, j.id).map(d => d.texto + ": " + d.puntos).join("\n")}" style="cursor:help"><b>${esBarbaros ? Math.round(rondasB * 20 / (App.opciones && App.opciones.coop ? 2 : 1)) : FWM.victoria.puntos(estado, datos, j.id)}</b> <small class="suave">?</small></td><td class="num">${FWM.estado.asentamientosDe(estado, j.id).length}</td><td class="num">${FWM.estado.tropasDe(estado, j.id).length}</td>
    ...
    const misPuntos = esBarbaros ? Math.round(rondasB * 20 / (App.opciones && App.opciones.coop ? 2 : 1)) : FWM.victoria.puntos(estado, datos, App.humano);
    ...
    if (C) {
      ...
      if (estado.ganador === App.humano && C.siguiente()) fila.appendChild(App.boton(T.campana.seguir, irCampana, "btn btn-primario"));
  ...
  function ceremonia(App, opciones, alAcabar) {
    const { estado, datos } = App; const T = datos.textos;
    const misPuntos = estado.barbaros ? Math.round(FWM.barbaros.rondas(estado) * 20 / (App.opciones && App.opciones.coop ? 2 : 1)) : FWM.victoria.puntos(estado, datos, App.humano);
    ...
    const recordAntes = (opciones.records && opciones.records[(coop ? "barbarosCoopRecord" : "barbarosRecord") + "Antes"]) || 0;
    const hPos = document.createElement("div"); hPos.className = "ceremonia-pos";
    hPos.textContent = esBarbaros ? rondas + "" : (primero ? T.victoria : (T.posiciones[pos] || (pos + 1) + ".º"));
    if (primero && !esBarbaros) hPos.classList.add("victoria"); // gana: la palabra, no el puesto
    caja.appendChild(hPos);
    if (esBarbaros) { const s2 = document.createElement("div"); s2.className = "ceremonia-sub"; s2.textContent = rondas > recordAntes ? T.barbaros.nuevoRecord.replace("{n}", rondas) : (coop ? T.coop.aguantasteis : T.barbaros.aguantaste).replace("{n}", rondas); caja.appendChild(s2); }
    ...
    (async () => {
      ...
      if (sub.nivel) {
        ...
        const t1 = document.createElement("div"); t1.className = "cer-nivel-txt"; t1.textContent = TH.nivelNuevo + " · " + ((datos.facciones[f] && datos.facciones[f].nombre) || ""); d.appendChild(t1);
        const t2 = document.createElement("div"); t2.className = "cer-nivel-nombre"; t2.textContent = (sub.nivel + " · " + dn.nombre).toUpperCase(); d.appendChild(t2);
        if (FWM.inicio && FWM.inicio.textoNivel) { const u = document.createElement("div"); u.className = "cer-nivel-unidad"; if (dn.hito === "unidad" || dn.hito === "curtida" || dn.hito === "maestra" || dn.hito === "techo") u.appendChild(FWM.iconos.canvasTropa(datos.facciones[f].unidad, "#2f6fd6", 48, false, f)); const ut = document.createElement("span"); ut.textContent = FWM.inicio.textoNivel(f, sub.nivel); u.appendChild(ut); d.appendChild(u); }
