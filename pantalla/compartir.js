...
FWM.compartir = (function () {
  ...
  function tarjeta(App, op) {
    ...
    ctx.fillText((FWM.heroes.nombreNivel(FWM.heroe.nivelVisible()) || "").toUpperCase(), LADO / 2, 778);
    // puntos, en grande
    ...
    const pie = [e.mapa.nombre, `${T.turno.toLowerCase()} ${e.limiteTurnos ? Math.min(e.turno, e.limiteTurnos) : e.turno}`, T.tipos[(App.opciones && App.opciones.tipo) || "rapida"]].filter(Boolean).join(" · ");
