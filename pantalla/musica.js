...
FWM.musica = (function () {
  ...
  let dormida = false, esperaBlur = null, apartada = false;
  // Notas de voz (15 sep 2026): mientras se graba o se escucha una, la música se aparta (con ella sonando no se
  // entendía nada y el micro la recogía). apartar(false) la devuelve donde estaba.
  function apartar(si) {
    if (si) { if (apartada) return; apartada = true; if (sonando && !dormida) { dormida = true; pausarTodos(); } return; }
    if (!apartada) return; apartada = false; despertar();
  }
  function dormir() {
  ...
  return { apartar, empezar, parar, volumen, alternar, diagnostico, saltar, activa: () => activa, sonando: () => sonando, escena: () => escena };
})();
