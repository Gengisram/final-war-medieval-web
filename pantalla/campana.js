...
FWM.campana = (function () {
  ...
  function de(id) {
    ...
    function opciones(n) {
      const cap = capitulo(n); if (!cap) return null;
      // 21 sep 2026: un capítulo puede traer su tipo de batalla, sus equipos, su objetivo y su guion
      return { tipo: "campana", campana: id, faccion: id, capitulo: cap.id, mapaHecho: cap.mapa, rivales: cap.rivales, dificultad: cap.dificultad, limite: cap.limite, hucha: cap.hucha || 2, batalla: cap.batalla || "conquista", equipos: cap.equipos, objetivo: cap.objetivo, guion: cap.guion, atacantes: cap.atacantes, tamanoEjercito: cap.ejercito, tecnologia: "todo", recursos: "equilibrado", semilla: 5000 + cap.id * 17 + (id === "castilla" ? 0 : [...id].reduce((s, c) => s * 31 + c.charCodeAt(0), 7) % 100000), jefe: cap.jefe || null, capital: cap.capital || null, enemigos: cap.enemigos ? cap.enemigos.map(e => Object.assign({ cultura: culturaEnemigo(id, e), nombresDe: culturaEnemigo(id, e) }, e)) : null };
    }
