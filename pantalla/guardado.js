...
FWM.guardado = (function () {
  ...
  function cargar() {
    try {
      ...
      // 21 sep 2026: la máquina de asedio pasó del id "trabuco" a "trabuquete" (ya se llamaba así en pantalla)
      for (const t of Object.values(d.estado.tropas || {})) if (t.tipo === "trabuco") t.tipo = "trabuquete";
      return d;
    } catch (e) { return null; }
  }
  ...
  function vacios() { return { resistirRecord: 0, partidas: 0, ganadas: 0, racha: 0, mejorRacha: 0, mejorVictoria: {}, historial: [], puntosTotal: 0, mejorPuntos: 0, rachaDias: 0, mejorRachaDias: 0, ultimoDia: null, ganadasPorBando: {}, medallas: {}, heroesMatados: 0, duelosGanados: 0, diasGanados: 0 }; }
  ...
  function anotarResultado(resultado, ctx) {
    ...
    if (resultado.tipo === "resistir") {
      // el récord de dobles va aparte: aguantar acompañado no es lo mismo que aguantar solo
      const clave = resultado.coop ? "resistirCoopRecord" : "resistirRecord";
      r[clave + "Antes"] = r[clave] || 0; r[clave] = Math.max(r[clave] || 0, resultado.rondas || 0);
    }
    if (resultado.gano && resultado.tipo === "campana" && FWM.campana) { const C = FWM.campana.de ? FWM.campana.de(resultado.campana) : FWM.campana; r.campana = Math.max(FWM.campana.mejorProgreso ? FWM.campana.mejorProgreso() : 0, C.progreso().hechos + (C.superado(resultado.capitulo) ? 0 : 1)); }
  ...
  // Facciones del mapa del día (13 sep 2026): una gratis y una de pago, que van cambiando cada día.
  function faccionesDelDia() {
    const n = Math.floor(Date.parse(new Date().toISOString().slice(0, 10)) / 864e5);
    const g = FWM.facciones.gratis(), p = FWM.facciones.dePago();
    return [g[n % g.length], p[n % p.length]];
  }
  ...
  return { guardar, cargar, borrar, records, anotarResultado, resumenPuntos, textoPuntos, ajustes, guardarAjustes, rachaViva, semillaDelDia, faccionesDelDia };
})();
