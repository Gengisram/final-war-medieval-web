...
FWM.paneles = (function () {
  ...
  function renderBarra(App) {
    ...
    if (estado.barbaros) {
      ...
      if (marc0) {
        ...
        if (coopM) {
          ...
          const defs = estado.jugadores.filter(x => !x.barbaros);
    ...
    turno.innerHTML = `${T.turno} <b>${estado.limiteTurnos ? Math.min(estado.turno, estado.limiteTurnos) + "/" + estado.limiteTurnos : estado.turno}</b> · ${nombreReino(App, activo)}` +
  ...
  function renderPoder(App) {
    ...
    if (!f || (j.nivel || 1) < FWM.facciones.PODER || estado.ganador != null || estado.barbaros && j.sinEliminar) { if (b) b.hidden = true; if (reg) reg.classList.remove("con-poder"); return; }
  ...
  function textoHabilidades(datos, obj) {
    ...
    for (const [k, v] of Object.entries(obj || {})) {
      ...
      if (k === "stats") { for (const [st, n] of Object.entries(v)) partes.push(st === "movimiento" ? H.movimiento.replace("{n}", n) : `+${n} ${T[st] ? T[st].toLowerCase() : st}`); }
  ...
  return { textoHabilidades, especialUnidad, renderPoder, pintarBotonMusica, renderBurbuja, posicionarBurbuja, renderRecluta, posicionarRecluta, renderBarra, renderPanel, renderRegistro, renderReino, textoEvento, aviso, nombreReino, motivo, botonAccion, graficaLinea };
})();
