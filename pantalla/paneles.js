...
FWM.paneles = (function () {
  // Puntos de movimiento -> casillas que puede andar (nunca medios: 23 sep 2026, se enseñaba "1,5" y no se entendía)
  function pasos(datos, puntos) { return String(FWM.stats.casillas(datos, puntos)); }
  ...
  function pintarBotonMusica() {
    ...
    for (const id of ["btn-musica", "btn-musica-inicio"]) {
  ...
  // Una línea con lo que hay que hacer para ganar esta partida.
  // "Conquista la Torre del Cuervo": el artículo del nombre de un pueblo va en minúscula dentro de la frase
  const enFrase = (nombre) => String(nombre || "").replace(/^(La|El|Los|Las|The) /, (m) => m.toLowerCase());
  function textoObjetivo(App, estado, datos) {
    const ob = estado.objetivo; const T = datos.textos.objetivos || {};
    // sin objetivo especial (conquista de siempre): también se dice, si no la barra no contaba nada (25 sep 2026)
    if (!ob) return estado.oleadas || (App.opciones && App.opciones.duelo) ? "" : (estado.limiteTurnos ? (T.conquista || "").replace("{n}", estado.limiteTurnos) : (T.conquistaSinLimite || "")); const miEquipo = FWM.victoria.equipoDe(estado.jugadores[App.humano]);
    if (ob.tipo === "aguantar") {
      const quedan = Math.max(0, ob.turnos - estado.turno + 1);
      const cap = ob.capital && (ob.hexCapital || ((estado.jugadores.find(x => FWM.victoria.equipoDe(x) === ob.equipo && x.capital) || {}).capital));
      const a = cap && estado.asentamientos[cap];
      if (a) return (ob.equipo === miEquipo ? T.aguantarCapital : T.romperCapital).replace("{n}", quedan).replace("{sitio}", enFrase(a.nombre));
      return (ob.equipo === miEquipo ? T.aguantar : T.romper || T.aguantar).replace("{n}", quedan);
    }
    if (ob.tipo === "tomar") {
      const a = estado.asentamientos[ob.hex];
      const mio = a && a.dueno != null && FWM.victoria.equipoDe(estado.jugadores[a.dueno]) === miEquipo;
      const dueno = a && a.dueno != null && estado.jugadores[a.dueno];
      // "el pueblo de los Lara", "el pueblo del señor de las minas": el artículo en minúscula dentro de la frase
      // "Conquista Alepo, el pueblo de la guardia de Alepo" repetía el nombre: si el dueño ya lo lleva, basta con el sitio
      const nomDueno = dueno ? nombreReino(App, dueno, true) : "";
      const nucleo = (x) => String(x || "").toLowerCase().replace(/^(the|el|la|los|las) /, "");
      const plantilla = mio ? T.defender : (a && nucleo(nomDueno).includes(nucleo(a.nombre)) && T.tomarSolo ? T.tomarSolo : T.tomar);
      return plantilla.replace("{sitio}", a ? enFrase(a.nombre) : "").replace("{dueno}", nomDueno)
        .replace(/\bde El /, "del ").replace(/\bde (Los|Las|La) /, (m, x) => "de " + x.toLowerCase() + " ").replace(/\bof The /, "of the ");
    }
    if (ob.tipo === "heroe") {
      // cuántos héroes rivales quedan: con dos o tres, "mata al héroe rival" en singular engañaba (25 sep 2026)
      const vivos = estado.jugadores.filter(j => !j.eliminado && FWM.victoria.equipoDe(j) !== miEquipo && j.heroeTropa && estado.tropas[j.heroeTropa]).length;
      return vivos > 1 && T.heroeN ? T.heroeN.replace("{n}", vivos) : (T.heroe || "");
    }
    if (ob.tipo === "clave") {
      const tengo = FWM.victoria.clavePorEquipo(estado, miEquipo);
      const seguidos = (estado.claveSeguidos && estado.claveSeguidos[miEquipo]) || 0;
      return (T.clave || "").replace("{n}", ob.n || 2).replace("{tengo}", tengo).replace("{turnos}", Math.max(0, (ob.turnos || 2) - seguidos));
    }
    if (ob.tipo === "escapar") return (ob.equipo === miEquipo ? (ob.lado && T.escaparLado && T.escaparLado[ob.lado] ? T.escaparLado[ob.lado] : ob.lado && T.escaparLejos ? T.escaparLejos : T.escapar) : T.cazar) || "";
    return "";
  }

  const escaparTxt = (t) => String(t == null ? "" : t).replace(/[&<>"]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[c]);

  // Los puntos del que va mejor entre los rivales: sin esto solo se veían los tuyos y no sabías si ibas ganando
  // (22 sep 2026, Rodrigo). En el duelo no hace falta: tiene su propio marcador con los dos.
  function puntosRival(App, estado, datos) {
    const T = datos.textos;
    const miEquipo = FWM.victoria.equipoDe(estado.jugadores[App.humano]);
    const rivales = estado.jugadores.filter(j => !j.eliminado && FWM.victoria.equipoDe(j) !== miEquipo);
    if (!rivales.length) return "";
    const mejor = Math.max(...rivales.map(j => FWM.victoria.puntos(estado, datos, j.id)));
    return ` · ${T.rival} <b>${mejor}</b>`;
  }
  ...
  function renderBarra(App) {
    ...
    // modo Resistir: la ronda manda, y se avisa de la oleada que viene
    if (estado.oleadas) {
      const sig = FWM.oleadas.siguienteOleada(estado);
      turno.innerHTML = `${T.resistir.ronda} <b>${FWM.oleadas.rondas(estado)}</b>` +
        (sig ? ` · <span class="suave">${T.resistir.proxima.replace("{ronda}", sig.ronda).replace("{lados}", sig.lados)}</span>` : "") +
        (App.opciones && App.opciones.coop ? ` <b id="barra-reloj" class="reloj">${FWM.duelo.texto()}</b>` : "");
      // a dobles, marcador de los dos (15 sep 2026): la ronda es de los dos, pero se ve cuántas tropas ha matado cada uno
      const marc0 = document.getElementById("barra-marcador");
      if (marc0) {
        const coopM = !!(App.opciones && App.opciones.coop);
        marc0.hidden = !coopM;
        if (coopM) {
          const st = estado.estadisticas || {};
          const defs = estado.jugadores.filter(x => !x.horda);
          const bajas = (x) => (st[x.id] && st[x.id].matadas) || 0;
          const total = defs.reduce((s, x) => s + bajas(x), 0);
          marc0.innerHTML = defs.map(x => `<span class="marc${x.id === App.humano ? " mio" : ""}${x.eliminado ? " caido" : ""}"><i style="background:${x.color}"></i>${escaparTxt(x.id === App.humano ? T.duelo.yo : (x.apodo || x.nombre))} <b>${bajas(x)}</b></span>`).join("")
            + `<span class="marc-pos">${T.coop.juntos} <b>${total}</b> ${T.coop.bajas}</span>`;
        }
      }
      document.getElementById("btn-chat").hidden = !(FWM.duelo.activo() && FWM.duelo.enLinea()); // a dobles, chat como en el duelo
    ...
    // objetivo de la partida (21 sep 2026): qué hay que hacer para ganar, siempre a la vista
    const obj = textoObjetivo(App, estado, datos);
    turno.innerHTML = (obj ? `<span class="barra-objetivo">${escaparTxt(obj)}</span> · ` : "") +
      `${T.turno} <b>${estado.limiteTurnos ? Math.min(estado.turno, estado.limiteTurnos) + "/" + estado.limiteTurnos : estado.turno}</b> · ${nombreReino(App, activo)}` +
      (enDueloBarra ? "" : ` · <b>${FWM.victoria.puntos(estado, datos, App.humano)}</b> ${T.puntos.toLowerCase()}${puntosRival(App, estado, datos)}`) +
  ...
  // Botón del poder del líder (13 sep 2026): abajo a la izquierda del mapa, con el escudo de tu facción.
  // Solo se ve si tu facción ya tiene el poder (nivel 5); se apaga cuando no quedan usos y brilla mientras dura.
  function renderPoder(App) {
    const { estado, datos } = App; const T = datos.textos;
    let b = document.getElementById("btn-poder");
    const j = estado && estado.jugadores[App.humano];
    const f = j && j.faccion && datos.facciones[j.faccion];
    // el registro de abajo a la izquierda deja sitio al botón (en el móvil se metía debajo, 14 sep 2026)
    const reg = document.getElementById("registro");
    if (!f || (j.nivel || 1) < FWM.facciones.PODER || estado.ganador != null || estado.oleadas && j.sinEliminar) { if (b) b.hidden = true; if (reg) reg.classList.remove("con-poder"); return; }
    if (reg) reg.classList.add("con-poder");
    if (!b) { b = document.createElement("button"); b.id = "btn-poder"; b.className = "boton-poder"; document.getElementById("mapa-cont").appendChild(b); }
    b.hidden = false;
    const usos = FWM.facciones.usosPoder(j); const activo = !!j.poderActivo; const miTurno = estado.jugadorActivo === App.humano;
    b.className = "boton-poder" + (activo ? " activo" : "");
    b.innerHTML = `${FWM.emblemas ? FWM.emblemas.svg(j.faccion, 26) : ""}<span>${f.poder.nombre}${usos > 1 ? " ×" + usos : ""}</span>`;
    b.title = activo ? T.poderes.activo.replace("{nombre}", f.poder.nombre) : f.poder.texto;
    b.disabled = !activo && (!miTurno || usos <= 0 || App.ocupado);
    b.onclick = () => {
      if (activo) { aviso(T.poderes.activo.replace("{nombre}", f.poder.nombre), 2500); return; }
      if (usos <= 0) { aviso(T.poderes.gastado, 2500); return; }
      App.confirmar(T.poderes.confirmar.replace("{nombre}", f.poder.nombre).replace("{texto}", f.poder.texto) + " " + (usos === 1 ? T.poderes.quedan : T.poderes.quedanVarios).replace("{n}", usos), () => { App.aplicar({ tipo: "poder" }); FWM.sonido.fanfarria && FWM.sonido.fanfarria(); });
    };
  }
  ...
  function textoEvento(App, e) {
    ...
    // un héroe se nombra por su líder ("Harald Hardrada se mueve"), no por su clase, que se confundía con la tropa
    // de siempre ("Espadachín se mueve", 14 sep 2026); si no se sabe de quién es, "Héroe (Espadachín)"
    const nombre = (tipo, jugadorId) => {
      const def = datos.tropas[tipo]; if (!def) return tipo;
      if (!def.heroe) return def.nombre;
      const j = jugadorId != null && App.estado && App.estado.jugadores[jugadorId];
      const f = j && j.faccion && datos.facciones[j.faccion];
      return f && f.clase === def.heroe ? f.lider : (T.heroe || "Héroe") + " (" + def.nombre + ")";
    };
    ...
    return plantilla
      ...
      .replace("{tropa}", nombre(e.tropa, e.jugador))
      .replace("{atacante}", nombre(e.atacante, e.jugador))
      .replace("{defensor}", nombre(e.defensor, e.jugadorDefensor))
      ...
      .replace("{oro}", e.oro != null ? e.oro : "")
      .replace("{poder}", e.faccion && datos.facciones && datos.facciones[e.faccion] ? datos.facciones[e.faccion].poder.nombre : "")
      .replace("{turno}", e.turno != null ? e.turno : "")
      .replace("{tropas}", (e.tropas || []).map(x => nombre(x, e.jugador)).join(", "))
      .replace("{clave}", e.clave && App.textoGuion ? App.textoGuion(e.clave) : "")
      .replace("{extra}", extra);
  }
  ...
  function renderRecluta(App) {
    ...
    const tipos = Object.keys(datos.tropas).filter(t => !datos.tropas[t].noReclutable && (def.recluta.includes("*") || def.recluta.includes(t)) && FWM.facciones.puedeTener(datos, j, t) == null);
    for (const tipo of tipos) {
      ...
      b.appendChild(FWM.iconos.canvasTropa(dt.icono, j.color, 40, false, j.faccion));
  ...
  function renderPanel(App) {
    ...
    panel.innerHTML = `<h2>${T.titulo}</h2>
      ...
      ${j.faccion && datos.facciones[j.faccion] ? `<p class="suave">${FWM.emblemas ? FWM.emblemas.svg(j.faccion, 16) : ""} ${datos.facciones[j.faccion].lider} · ${T.facciones.nivel.replace("{n}", FWM.facciones.nivelDe(j))}<br>${T.rasgo}: <b>${datos.facciones[j.faccion].rasgo.nombre}</b> ${datos.facciones[j.faccion].rasgo.texto}<br>${T.facciones.poder}: <b>${datos.facciones[j.faccion].poder.nombre}</b> ${FWM.facciones.nivelDe(j) >= FWM.facciones.PODER ? datos.facciones[j.faccion].poder.texto : T.poderes.bloqueado}</p>` : datos.bandos[j.bando] ? `<p class="suave">${T.rasgo}: ${datos.bandos[j.bando].rasgo}</p>` : ""}
  ...
  // Texto de las habilidades de una unidad propia ({ firme: 10 }, { bonos: { montada: 10 } }…) (13 sep 2026).
  function textoHabilidades(datos, obj) {
    const T = datos.textos; const H = T.facciones.habilidades; const et = (e) => (T.etiquetasNombre[e] || e);
    const partes = [];
    for (const [k, v] of Object.entries(obj || {})) {
      if (v == null || (typeof v === "object" && !Object.keys(v).length)) continue;
      if (k === "stats") { for (const [st, n] of Object.entries(v)) partes.push(st === "movimiento" ? H.movimiento.replace("{n}", FWM.stats.casillas(datos, 2 + n) - 1) : `+${n} ${T[st] ? T[st].toLowerCase() : st}`); }
      else if (k === "bonos" || k === "defensaContra") { for (const [e, n] of Object.entries(v)) partes.push(H[k].replace("{n}", n).replace("{etiqueta}", et(e))); }
      else if (k === "furia") partes.push(H.furia.replace("{n}", v.valor).replace("{v}", v.porVida).replace("{t}", v.tope));
      else if (k === "trasAtaque") partes.push(H.trasAtaque);
      else if (H[k]) partes.push(H[k].replace("{n}", v));
    }
    return partes.join(" · ");
  }
  // Lo que hace de especial una unidad propia al nivel de su dueño.
  function especialUnidad(datos, tipo, nivel) {
    const dt = datos.tropas[tipo]; if (!dt || !dt.faccion) return "";
    const TF = datos.textos.facciones;
    const partes = [textoHabilidades(datos, { firme: dt.firme, furia: dt.furia, defensaContra: dt.defensaContra })].filter(Boolean);
    if (nivel >= FWM.facciones.CURTIDA) partes.push(textoHabilidades(datos, dt.curtida));
    if (nivel >= FWM.facciones.MAESTRA) partes.push(textoHabilidades(datos, dt.maestra));
    return partes.join(" · ");
  }
  ...
  function statsDe(App, tr) {
    ...
    detalle.innerHTML = `<div class="stats"><div>${T.asedio}<b>${st("asedio")}</b></div><div>${T.alcance}<b>${st("alcance")}</b></div>${atkEf !== st("ataque") ? `<div>${T.ataque} <small class="suave">${T.sano}</small><b>${st("ataque")}</b></div>` : ""}</div>
      ...
      <p><span class="suave">${T.mantenimiento}: ${FWM.economia.mantenimientoDe(datos, tr, estado)} ${T.porTurno}</span></p>`;
    // unidad propia: lo que hace de especial a su nivel (se ve siempre, también la del rival)
    const especial = especialUnidad(datos, tr.tipo, (estado.jugadores[tr.dueno] && estado.jugadores[tr.dueno].nivel) || 1);
    if (especial) { const pe = document.createElement("p"); pe.className = "sobre"; pe.innerHTML = `<b>${T.fichas.especial}</b>: ${especial}`; cont.appendChild(pe); }
  ...
  function panelTropa(App, tr) {
    ...
    cab.appendChild(dt.heroe ? FWM.figuras.canvasHeroe(Object.assign({ faccion: j.faccion }, j.heroe), j.color, 52, false, tr.dueno !== App.humano) : FWM.iconos.canvasTropa(dt.icono, j.color, 52, tr.dueno !== App.humano, j.faccion));
  ...
  function renderReino(App) {
    ...
    for (const k of Object.keys(cuentaT)) {
      const fila = document.createElement("div"); fila.className = "reino-fila";
      fila.appendChild(FWM.iconos.canvasTropa(datos.tropas[k].icono, j.color, 26, false, j.faccion));
      const sp = document.createElement("span");
      ...
      fila.title = `${T.mantenimiento} ${T.fichas.mantCadaUna.replace("{n}", datos.tropas[k].mantenimiento)}`;
      fila.appendChild(sp);
      lT.appendChild(fila);
    }
  ...
  return { textoObjetivo, textoHabilidades, especialUnidad, renderPoder, pintarBotonMusica, renderBurbuja, posicionarBurbuja, renderRecluta, posicionarRecluta, renderBarra, renderPanel, renderRegistro, renderReino, textoEvento, aviso, nombreReino, motivo, botonAccion, graficaLinea };
})();
