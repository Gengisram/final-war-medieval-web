...
FWM.inicio = (function () {
  ...
  function estadoFaccion(f) {
    ...
    if (FWM.heroe.tiene(f)) return { tuya: true, texto: (d.pago ? TF.tuya : TF.gratis) + " · " + TF.nivel.replace("{n}", FWM.heroe.nivelVisible(f)) };
  ...
  function vistaFaccion(f, atras, op) {
    ...
    const desc = document.createElement("p"); desc.className = "faccion-desc"; desc.textContent = d.descripcion; cont.appendChild(desc);
    ...
    // rasgo y poder
    const sec = (titulo) => { const x = document.createElement("section"); x.className = "reino-sec"; x.innerHTML = `<h3>${titulo}</h3>`; cont.appendChild(x); return x; };
    ...
    if (b) {
      ...
      for (const [k, nom] of filas) { const a = u.stats[k] || 0, c = b.stats[k] || 0; if (!a && !c) continue; html += `<tr><td>${nom}</td><td><b>${a}</b>${flecha(a, c)}</td><td class="suave">${c}</td></tr>`; }
    ...
    // el héroe (el líder, con su clase)
    const c = App.datos.heroes.clases[d.clase]; const sh = sec(TF.heroe);
    sh.insertAdjacentHTML("beforeend", `<p style="margin:0"><b>${escapar(d.lider)}</b> (${escapar(c.nombre)}): ${escapar(c.descripcion)}<br><small class="suave">${escapar(c.rasgo)}</small></p>`);
    // la escalera
  ...
  function vistaBarbaros() {
    ...
    card.appendChild(selectorFaccion(() => vistaBarbaros()));
    ...
    fila.appendChild(App.boton(TB.jugar, () => {
      App.nuevaPartida({ tipo: "barbaros", mapaHecho: "arena", faccion: FWM.heroe.faccion(), rivales: 1, dificultad: "normal", tecnologia: "todo", hucha: 2, recursos: "equilibrado", limite: 0, semilla: Math.floor(Math.random() * 1e6) + 1 });
    }, "btn btn-primario"));
  ...
  function pintarMejoras(sec, h, pr) {
    ...
    const pintarStats = () => {
      ...
      for (const k of ["ataque", "defensa", "vida", "movimiento", "alcance", "asedio"]) {
        const base = clase.stats[k] || 0; const v = base + (k === "alcance" ? 0 : (ef[k] || 0)); const extra = k === "alcance" ? 0 : (conMejora[k] || 0) - (ef[k] || 0);
    ...
    const pintarConfirmar = () => {
      ...
      sec.querySelectorAll(".heroe-mejora").forEach(x => { const m = D.mejoras[x.dataset.id]; const q = x.querySelector(".hm-puntos"); if (m && q) q.innerHTML = puntitos(x.dataset.id, h.mejoras[x.dataset.id] || 0, m.peldanos); });
      pintarStats();
      if (!elegida) return;
  ...
  // Un código de amigo lleva su modo (lo elige quien lo crea): dobles de Bárbaros o duelo.
  function arrancarPorCodigo(ses) {
    if (ses.coop) App.nuevaPartida({ tipo: "barbaros", coop: ses, mapaHecho: "arena", faccion: FWM.heroe.faccion(), rivales: 2, dificultad: "normal", tecnologia: "todo", hucha: 2, recursos: "equilibrado", limite: 0, semilla: ses.semilla });
  ...
  function vistaDuelo(op) {
    ...
    const empezar = (ses) => {
      if (ses.coop) { arrancarPorCodigo(ses); return; } // el código era de Bárbaros a dobles: se entra en dobles
  ...
  return { textoNivel: (f, n) => textoNivel(f, n), mostrar, ocultar, visible, refrescar, vistaCuenta, vistaRanking, vistaHeroe, vistaDuelo, vistaBienvenida, vistaCampana, vistaBatalla, vistaEscenarios, vistaBarbaros, vistaFacciones, vistaFaccion, vistaLeyendas };
})();
