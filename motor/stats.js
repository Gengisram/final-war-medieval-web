...
FWM.stats = (function () {
  ...
  function statHeroe(estado, datos, tropa, nombre, def) {
    ...
    // aura del Kan: los jinetes pegados a él mueven más (movimientoMontada) o pegan más (ataqueMontada, desde el
    // 23 sep 2026: con +1 casilla su caballería llegaba a tres, y a caballo el techo es dos)
    if ((nombre === "movimiento" || nombre === "ataque") && (def.etiquetas || []).includes("montada")) {
      const aura = auraSobre(estado, datos, tropa);
      if (aura && nombre === "movimiento" && aura.movimientoMontada) v += aura.movimientoMontada;
      if (aura && nombre === "ataque" && aura.ataqueMontada) v += aura.ataqueMontada;
    }
  ...
  // Lo que se enseña al jugador: casillas, no puntos. Con la regla del "paso más" (si te queda algún punto
  // puedes dar un paso aunque cueste más), 2 puntos = 1 casilla, 3 = 2, 4 = 2, 5 = 3.
  function casillas(datos, puntos) { return Math.ceil(Math.max(0, puntos || 0) / ((datos.reglas && datos.reglas.puntosPorPaso) || 2)); }

  return { casillas, tieneTec, nivelTec, efectos, statTropa, auraSobre, propAsentamiento, produccionAsentamiento, produccionYacimiento, vidaMax, ataqueEfectivo, tecDisponible, costeTec, nivelExperiencia, costeTropa, costeAsentamiento, factorExperiencia, bonoContra, defensaContra };
})();
