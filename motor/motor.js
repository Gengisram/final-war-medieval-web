...
FWM.motor = (function () {
  ...
  function aplicar(estado, datos, accion) {
    ...
    // si el que pasa a jugar acaba de quedar eliminado (se le fue la última tropa por falta de oro al empezar su
    // turno), el turno pasa al siguiente: nunca le toca a un eliminado (14 sep 2026)
    for (let vueltas = 0; nuevo.ganador == null && nuevo.jugadores[nuevo.jugadorActivo] && nuevo.jugadores[nuevo.jugadorActivo].eliminado && vueltas < nuevo.jugadores.length; vueltas++) {
      eventos.push(...(FWM.acciones.acciones.finTurno.ejecutar(nuevo, datos, { tipo: "finTurno" }) || []));
      eventos.push(...FWM.victoria.comprobar(nuevo, datos));
    }
  ...
  function accionesPosibles(estado, datos, tropaId) {
    ...
    if (!t || t.dueno !== estado.jugadorActivo || estado.ganador != null) return r;
    // tras atacar, las que pueden (mameluco, arquero a caballo, Kan) solo pueden moverse
    if (t.accionUsada) { if (t.trasAtaque && t.movRestante > 0) r.mover = FWM.acciones.alcanzablesDe(estado, datos, t); return r; }
  ...
  function prever(estado, datos, tropaId, objetivo) {
    ...
    // estacas: la caballería que carga contra un arquero largo maestro se clava
    const defD = FWM.facciones ? FWM.facciones.defDe(estado, datos, d) : datos.tropas[d.tipo];
    const estacas = FWM.stats.statTropa(estado, datos, t, "alcance") === 0 && defD.estacas && (datos.tropas[t.tipo].etiquetas || []).includes("montada") ? defD.estacas : 0;
    const recibesTotal = estacas ? (recibes ? [recibes[0] + estacas, recibes[1] + estacas] : [estacas, estacas]) : recibes;
    return { haces, recibes: recibesTotal, estacas, defensor: d, asentamiento: obj.asentamiento, vidaDefensor: d.vida, defensaDefensor: def, bono, bonoDef, bonoRecibes: FWM.stats.statTropa(estado, datos, t, "alcance") === 0 ? FWM.stats.bonoContra(datos, d, t, estado) : null };
  }
