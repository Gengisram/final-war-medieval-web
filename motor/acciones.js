...
FWM.acciones = (function () {
  ...
  function costeEntrar(estado, datos, tropa, v, pasar) {
    const h = estado.mapa.hexes[v];
    if (!h) return null;
    let coste = costeTerreno(estado, datos, v);
    if (coste == null) return null;
    // Estepa (mongoles): los jinetes pagan el bosque y la colina como llanura
    const llano = FWM.facciones && FWM.facciones.rasgo(estado, tropa.dueno, "terreno_llano");
    if (llano && (datos.tropas[tropa.tipo].etiquetas || []).includes(llano.etiqueta) && (!llano.terrenos || llano.terrenos.includes(h.terreno))) coste = Math.min(coste, datos.terrenos.llanura.costeMovimiento);
    // una tropa tuya no corta el camino: se pasa a través de ella, pero no se puede acabar encima (24 sep 2026,
    // Rodrigo). Las enemigas (y las de otros jugadores) sí lo cortan.
    const ocupa = E().tropaEn(estado, v);
    if (ocupa && (ocupa.id === tropa.id || !(pasar && ocupa.dueno === tropa.dueno))) return null; // ella misma no: volvería a su casilla
    if (h.construccion === "asentamiento") {
      ...
      if (E().aliados(estado, a.dueno, tropa.dueno)) return null; // el pueblo del compañero no se toma
      return a.guarnicion.length === 0 && a.integridad <= 0 ? coste : null; // conquista andando: sin guarnición y con brecha
    }
    return coste;
  }

  // Adónde puede llegar: sin las casillas ocupadas por tropas tuyas (se pasa por ellas, pero no se para).
  function alcanzablesDe(estado, datos, tropa) {
    const r = alcanzablesConPaso(estado, datos, tropa);
    for (const k of Object.keys(r)) if (E().tropaEn(estado, k)) delete r[k];
    return r;
  }
  // Todas las casillas por las que puede ir, también las de sus compañeros (para saber el camino).
  function alcanzablesConPaso(estado, datos, tropa) {
    const origen = E().posicionTropa(estado, tropa);
    const r = H().alcanzables(origen, tropa.movRestante,
      v => costeEntrar(estado, datos, tropa, v, true),
    ...
    for (const k of desdeDonde) {
      ...
      for (const v of H().vecinos(k)) {
        const c = costeEntrar(estado, datos, tropa, v, true);
  ...
  function conquistar(estado, datos, hex, jugadorId, tropa, eventos) {
    ...
    // Saqueo (vikingos): se llevan el oro del asentamiento según lo que era
    const saqueo = FWM.facciones && FWM.facciones.rasgo(estado, jugadorId, "saqueo");
    if (saqueo && antiguo != null) { const oro = saqueo.oro[tipoAntes] || 0; if (oro) { j.hucha.oro = (j.hucha.oro || 0) + oro; eventos.push({ tipo: "saqueo", jugador: jugadorId, oro, hex, asentamiento: a.nombre }); } }
  ...
  function objetivoEn(estado, datos, jugadorId, hex) {
    ...
    if (t && E().enemigos(estado, t.dueno, jugadorId)) return { tropa: t, asentamiento: null };
    const a = estado.asentamientos[hex];
    if (a && E().enemigos(estado, a.dueno, jugadorId) && a.guarnicion.length) {
  ...
  // Premios de matar (13 sep 2026): el berserker curtido se cura y el Mansa cobra su botín.
  function alMatar(estado, datos, asesino, hex, eventos) {
    if (!asesino || !estado.tropas[asesino.id]) return;
    const def = FWM.facciones ? FWM.facciones.defDe(estado, datos, asesino) : datos.tropas[asesino.tipo];
    if (def.curaAlMatar) { const max = S().vidaMax(estado, datos, asesino); const antes = asesino.vida; asesino.vida = Math.min(max, asesino.vida + def.curaAlMatar); if (asesino.vida > antes) eventos.push({ tipo: "cura", hex: E().posicionTropa(estado, asesino), cantidad: asesino.vida - antes, tropa: asesino.tipo, jugador: asesino.dueno }); }
    if (def.botin) { const j = estado.jugadores[asesino.dueno]; j.hucha.oro = (j.hucha.oro || 0) + def.botin; eventos.push({ tipo: "botin", jugador: asesino.dueno, oro: def.botin, hex: hex || E().posicionTropa(estado, asesino) }); }
  }

  // Un hexágono sin asentamiento pasa al que lo pisa.
  function pisar(estado, hex, jugadorId) { const h = estado.mapa.hexes[hex]; if (h && h.construccion !== "asentamiento" && !(h.dueno != null && h.dueno !== jugadorId && E().aliados(estado, h.dueno, jugadorId))) h.dueno = jugadorId; }
  ...
  acciones.mover = {
    ...
    validar(estado, datos, a) {
      // tras atacar, el mameluco curtido, el arquero a caballo curtido y el Kan aún pueden moverse (sin conquistar)
      const t0 = estado.tropas[a.tropa];
      const r = t0 && t0.trasAtaque && t0.dueno === estado.jugadorActivo ? { t: t0 } : tropaPropia(estado, a); if (r.error) return r.error;
      const t = r.t;
      if (t.movRestante <= 0) return "sin_movimiento";
      const alc = alcanzablesDe(estado, datos, t);
      if (!alc[a.a]) return "fuera_de_alcance";
      if (t.trasAtaque && estado.asentamientos[a.a] && estado.asentamientos[a.a].dueno !== t.dueno) return "no_tras_atacar";
      return null;
    },
    ejecutar(estado, datos, a) {
      const eventos = [];
      const t = estado.tropas[a.tropa];
      const alc = alcanzablesConPaso(estado, datos, t); // con el camino entero, aunque cruce a un compañero
      ...
      // Pisar un hexágono lo hace tuyo (5 sep 2026: fuera el botón Reclamar), y eso vale para TODOS los
      // que se cruzan por el camino, no solo el de destino: si mueves 3 y avanzas 3, reclamas las tres.
      // Los asentamientos se saltan: esos se toman entrando (conquistar), no pisando (8 sep 2026).
      const defM = FWM.facciones ? FWM.facciones.defDe(estado, datos, t) : datos.tropas[t.tipo];
      for (const k of H().ruta(alc, a.a)) {
        const hk = estado.mapa.hexes[k];
        if (!hk || hk.construccion === "asentamiento") continue;
        // jinete mandinga maestro: oro por cada hexágono enemigo que pisa
        if (defM.oroPorHexEnemigo && E().enemigos(estado, hk.dueno, t.dueno) && estado.jugadores[hk.dueno]) { const jm = estado.jugadores[t.dueno]; jm.hucha.oro = (jm.hucha.oro || 0) + defM.oroPorHexEnemigo; eventos.push({ tipo: "botin", jugador: t.dueno, oro: defM.oroPorHexEnemigo, hex: k }); }
        pisar(estado, k, t.dueno);
      }
      if (h.construccion === "asentamiento") {
      ...
      if (t.trasAtaque && t.movRestante <= 0) t.trasAtaque = false;
  ...
  acciones.reclamar = {
    ...
    validar(estado, datos, a) {
      ...
      if (h.construccion || E().aliados(estado, h.dueno, t.dueno)) return "no_se_puede";
      return null;
    },
    ejecutar(estado, datos, a) {
      const t = estado.tropas[a.tropa];
  ...
  acciones.atacar = {
    ...
    ejecutar(estado, datos, a) {
      ...
      eventos.push({ tipo: "ataque", atacante: t.tipo, defensor: d.tipo, dano: g1.dano, ajuste: g1.ajuste, jugador: t.dueno, jugadorDefensor: d.dueno, hex: a.objetivo });
      ...
      if (claseH && claseH.fuego) {
        for (const v of FWM.hex.vecinos(a.objetivo)) {
          const x = E().tropaEn(estado, v); if (!x || !E().enemigos(estado, x.dueno, t.dueno)) continue;
          x.vida -= claseH.fuego.dano; eventos.push({ tipo: "fuego", hex: v, dano: claseH.fuego.dano, defensor: x.tipo, jugador: x.dueno });
          if (x.vida <= 0) { anotarMuerte(estado, datos, x, t.dueno); Ec().eliminarTropa(estado, x); eventos.push({ tipo: "muere", tropa: x.tipo, jugador: x.dueno, hex: v, por: t.dueno }); if (ex) darExperiencia(datos, t, ex.porMuerte, estado); alMatar(estado, datos, t, v, eventos); } // matadas/perdidas las cuenta motor.contar con el evento (antes se sumaban dos veces)
        }
      }
      // estacas del arquero largo maestro: la caballería que le carga se clava 10 (y puede morir en ellas)
      const defD = FWM.facciones ? FWM.facciones.defDe(estado, datos, d) : datos.tropas[d.tipo];
      let clavado = false;
      if (cuerpo && defD.estacas && (datos.tropas[t.tipo].etiquetas || []).includes("montada")) {
        t.vida -= defD.estacas; eventos.push({ tipo: "estacas", hex: E().posicionTropa(estado, t), dano: defD.estacas, tropa: t.tipo, jugador: t.dueno });
        if (t.vida <= 0) {
          const hexT = E().posicionTropa(estado, t); clavado = true;
          anotarMuerte(estado, datos, t, d.dueno); Ec().eliminarTropa(estado, t);
          eventos.push({ tipo: "muere", tropa: t.tipo, jugador: t.dueno, hex: hexT, por: d.dueno });
        }
      }
      if (clavado) { if (d.vida <= 0) { anotarMuerte(estado, datos, d, t.dueno); Ec().eliminarTropa(estado, d); eventos.push({ tipo: "muere", tropa: d.tipo, jugador: d.dueno, hex: a.objetivo, por: t.dueno }); } }
      else if (d.vida <= 0) {
        ...
        alMatar(estado, datos, t, a.objetivo, eventos);
        ...
        if (t.vida <= 0) {
          ...
          alMatar(estado, datos, d, hexT, eventos);
      ...
      if (estado.tropas[t.id]) {
        t.accionUsada = true;
        // tras atacar: el mameluco curtido se retira (sus puntos), el arquero a caballo curtido y el Kan siguen con lo que tenían
        const tras = (FWM.facciones ? FWM.facciones.defDe(estado, datos, t) : datos.tropas[t.tipo]).trasAtaque;
        t.movRestante = tras === "resto" ? Math.max(0, movAntes) : typeof tras === "number" ? tras : 0;
        t.trasAtaque = t.movRestante > 0 && !t.acuarteladaEn;
        if (!t.trasAtaque) t.movRestante = 0;
      }
  ...
  acciones.asediar = {
    ...
    validar(estado, datos, a) {
      ...
      if (!asent || !E().enemigos(estado, asent.dueno, t.dueno)) return "no_hay_objetivo";
    ...
    ejecutar(estado, datos, a) {
      ...
      if (!cuerpo && S().statTropa(estado, datos, t, "ataque") > 0) {
        ...
        if (obj && obj.tropa) {
          ...
          const g1 = golpe(estado, datos, t, d);
          eventos.push({ tipo: "ataque", atacante: t.tipo, defensor: d.tipo, dano: g1.dano, ajuste: g1.ajuste, jugador: t.dueno, jugadorDefensor: d.dueno, hex: a.objetivo });
          // (14 sep 2026) también anota la muerte: si era el héroe, cae de verdad (pócima, puntos por héroe abatido)
          if (d.vida <= 0) { anotarMuerte(estado, datos, d, t.dueno); Ec().eliminarTropa(estado, d); eventos.push({ tipo: "muere", tropa: d.tipo, jugador: d.dueno, hex: a.objetivo, por: t.dueno }); if (datos.reglas && datos.reglas.experiencia) darExperiencia(datos, t, datos.reglas.experiencia.porMuerte, estado); alMatar(estado, datos, t, a.objetivo, eventos); }
        }
      }
      ...
      if (cuerpo && datos.reglas && datos.reglas.asedioCuerpoACuerpoRecibeContraataque) {
        ...
        if (obj && obj.tropa && contraataca(datos, obj.tropa)) {
          ...
          if (t.vida <= 0) {
            const hexT = E().posicionTropa(estado, t);
            anotarMuerte(estado, datos, t, obj.tropa.dueno);
            Ec().eliminarTropa(estado, t);
            eventos.push({ tipo: "muere", tropa: t.tipo, jugador: t.dueno, hex: hexT, por: obj.tropa.dueno });
            alMatar(estado, datos, obj.tropa, hexT, eventos);
          }
        }
      }
      return eventos;
  ...
  acciones.reclutar = {
    ...
    validar(estado, datos, a) {
      ...
      { const e = FWM.facciones.puedeTener(datos, j, a.que); if (e) return e; } // facción y nivel (13 sep 2026)
  ...
  acciones.mejorarTropa = {
    ...
    validar(estado, datos, a) {
      ...
      { const e = FWM.facciones.puedeTener(datos, j, a.que); if (e) return e; }
  ...
  // Poder del líder (13 sep 2026): se abre en el nivel 5 de la facción, una vez por partida (dos en el nivel 10).
  // No gasta tropa. Las Navas, Furia del norte y Lluvia de flechas duran hasta tu próximo turno; Los griots,
  // Hattin y El Yam hacen efecto al momento. (La peregrinación daba oro y en las simulaciones desequilibraba
  // todo: el oro de golpe se convierte en tropas y en puntos. 13 sep 2026.)
  acciones.poder = {
    reversible: false,
    validar(estado, datos) {
      const j = estado.jugadores[estado.jugadorActivo];
      if (!j || !j.faccion || !datos.facciones[j.faccion]) return "no_se_puede";
      if ((j.nivel || 1) < FWM.facciones.PODER) return "nivel_heroe";
      if (FWM.facciones.usosPoder(j) <= 0) return "poder_gastado";
      if (j.poderActivo) return "poder_en_marcha";
      return null;
    },
    ejecutar(estado, datos) {
      const eventos = [];
      const j = estado.jugadores[estado.jugadorActivo]; const poder = datos.facciones[j.faccion].poder; const ef = poder.efecto || {};
      j.poderUsos = (j.poderUsos || 0) + 1;
      eventos.push({ tipo: "poder", jugador: j.id, poder: poder.id, faccion: j.faccion });
      if (ef.ataque || ef.defensa || ef.alcance || ef.inviernoEnemigos) j.poderActivo = { id: poder.id, turno: estado.turno };
      if (ef.turnosDeOro || ef.minimo) {
        const oro = Math.max(ef.minimo || 0, (Ec().resumen(estado, datos, j.id).ingresos.oro || 0) * (ef.turnosDeOro || 0));
        j.hucha.oro = (j.hucha.oro || 0) + oro; eventos.push({ tipo: "botin", jugador: j.id, oro, hex: j.capital });
      }
      if (ef.movimiento) for (const t of E().tropasDe(estado, j.id)) if (!t.accionUsada) t.movRestante += ef.movimiento;
      // cura: a todo el ejército, o solo a lo que esté cerca del héroe si el poder lleva "cerca" (los griots
      // viajan con el Mansa: no cantan para un ejército desperdigado por todo el mapa, 22 sep 2026)
      const heroeCura = ef.cerca ? E().posicionTropa(estado, estado.tropas[j.heroeTropa] || {}) : null;
      if (ef.curaTodas) for (const t of E().tropasDe(estado, j.id)) {
        if (ef.cerca) { const p = E().posicionTropa(estado, t); if (!p || !heroeCura || H().distancia(p, heroeCura) > ef.cerca) continue; }
        const max = S().vidaMax(estado, datos, t); const antes = t.vida; t.vida = Math.min(max, t.vida + ef.curaTodas);
        if (t.vida > antes) eventos.push({ tipo: "cura", hex: E().posicionTropa(estado, t), cantidad: t.vida - antes, tropa: t.tipo, jugador: j.id });
      }
      if (ef.danoPegados) {
        const mias = E().tropasDe(estado, j.id).map(t => E().posicionTropa(estado, t)).filter(Boolean);
        for (const x of Object.values(estado.tropas)) {
          if (!E().enemigos(estado, x.dueno, j.id) || !x.hex) continue;
          if (!mias.some(p => H().distancia(p, x.hex) === 1)) continue;
          const antes = x.vida; x.vida = Math.max(1, x.vida - ef.danoPegados);
          if (x.vida < antes) eventos.push({ tipo: "sed", hex: x.hex, dano: antes - x.vida, tropa: x.tipo, jugador: x.dueno });
        }
      }
      return eventos;
    },
  ...
  acciones.finTurno = {
    ...
    ejecutar(estado, datos) {
      ...
      // guion de la partida (refuerzos, traiciones, objetivos que cambian): modo/guion.js. Se mira ya con la ronda
      // nueva: "turno 4" es al EMPEZAR el turno 4, antes de que juegues (24 sep 2026; antes era al acabar tu turno 4
      // y los refuerzos salían a la vista de las máquinas antes de que pudieras moverlos)
      if (FWM.guion) eventos.push(...FWM.guion.revisar(estado, datos));
