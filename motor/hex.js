...
FWM.hex = (function () {
  ...
  function ruta(alcanzados, destino) {
    const camino = [];
    let c = destino;
    const vistos = new Set(); // por si acaso: un camino con vuelta colgaba el juego (24 sep 2026)
    while (c && alcanzados[c] && !vistos.has(c)) { vistos.add(c); camino.unshift(c); c = alcanzados[c].desde; }
    return camino;
  }
