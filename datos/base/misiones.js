...
FWM.datosBase.misiones = {
  diarias: [
    ...
    { id: "clave4", texto: "Termina la partida con 4 puntos clave", oro: 12, cumple: (r) => (r.puntosClave || 0) >= 4 },
    ...
    { id: "clave2", texto: "Termina la partida con 2 puntos clave", oro: 6, cumple: (r) => (r.puntosClave || 0) >= 2 },
  ...
  semanales: [
    ...
    { id: "s_bandos3", texto: "Gana con 2 facciones distintas", oro: 50, bandos: true, meta: 2 },
...
FWM.misiones = (function () {
  ...
  // "Gana con…": una facción que tengas (13 sep 2026: antes era cualquier bando y podía tocar uno imposible)
  function bandoDelDia(datos) { const ids = (FWM.facciones ? FWM.facciones.lista() : Object.keys(datos.bandos)).filter(f => !FWM.heroe || FWM.heroe.tiene(f)); const g = FWM.azar.crear(semillaDe(hoyClave()) + 7); const f = ids[Math.floor((g.siguiente ? g.siguiente() : Math.random()) * ids.length)]; return (datos.facciones && datos.facciones[f]) ? datos.facciones[f].bando : f; }
