...
FWM.ia = (function () {
  ...
  function jugar(estado, datos, opciones) {
    ...
    if (!ctx.estado.jugadores[ctx.yo].eliminado && ctx.estado.ganador == null) {
      ...
      construir(ctx);
      reclutar(ctx);
      if (FWM.iaComun) FWM.iaComun.golpeConjunto(ctx); // entre todas, a por quien acosa la casa (16 sep 2026)
      moverTropas(ctx);
    }
    hacer({ tipo: "finTurno" });
  ...
  function construir(ctx) {
    ...
    if (j.capital && castillos < Math.max(1, Math.floor(otros / 2)) && (j.hucha.oro || 0) >= 45) {
  ...
  function elegirObjetivo(estado, datos, yo, t, quieroFundar) {
    ...
    if (militar || !candidatos.length) {
      ...
      // el que acosa mi casa (a 2 o menos de un asentamiento mío) va primero: antes se iban a por minas lejanas
      // mientras el héroe rival rodeaba la capital (16 sep 2026)
      const misCasas = E().asentamientosDe(estado, yo).map(x => x.hex);
      for (const e of Object.values(estado.tropas)) {
        if (e.dueno === yo || !e.hex) continue;
        const acosa = misCasas.some(m => H().distancia(m, e.hex) <= 2);
        candidatos.push({ hex: e.hex, prioridad: acosa ? 5 : 2 });
      }
    }
